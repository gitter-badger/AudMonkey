...
function cleanfulltree {
   ...
   myrmvf $1 src/audmonkey src/.depend src/.gchdepend
...
function slimtree {
   ...
   printf "Removing developer scripts not needed to build audmonkey ... "
   myrmrvf $1 scripts/mw2html_audmonkey 
   printf "Done\n"
...
echo "Maketarball 2.1.0 -- make an AudMonkey distribution tarball"

# check number of arguments, if not one then print a usage message
if [ $# -ne 1 ] ; then
   echo "Script to make directory trees for audmonkey source tarballs"
   echo "Usage: $0 <mode>"
...
if [ ! -f "src/AudMonkey.h" ] ; then
   echo "$0 must be run from top-level audmonkey directory"
   exit 1
fi
...
tmpsrc="${topdir}/$(mktemp -d audmonkey-src-XXXXXX)" # where initial modifications are done
...
# (because we want the same one that will be used to build AudMonkey). This is a
...
# you actually need to build audmonkey on a shared library system with the
...
tarname="audmonkey-minsrc-${version}" # the directory we will find inside tarballs
mv "${tmpsrc}" "${tarname}"
printf "Done\n"
