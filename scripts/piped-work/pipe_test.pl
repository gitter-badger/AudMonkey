...
# Test script for communicating with audmonkey via mod-script-pipe
# AudMonkey should be running first, with the scripting plugin loaded.
#
...
# on the 'Scripting' page of the AudMonkey wiki.
...
# TODO: Maybe get the pipe names from audmonkey?
if ($^O eq 'MSWin32') {
   $Name = 'Srv';
   ...
   $UID = $<;
   $ToSrvName = '/tmp/audmonkey_script_pipe.to.'.$UID;
   $FromSrvName = '/tmp/audmonkey_script_pipe.from.'.$UID;
} elsif ($^O eq 'darwin') {
   $UID = $<;
   $ToSrvName = '/tmp/audmonkey_script_pipe.to.'.$UID;
   $FromSrvName = '/tmp/audmonkey_script_pipe.from.'.$UID;
...
sub testClearAndPasters{
   ...
   # This preferences is not read in AudMonkey 2.2.x where duration
...
# A list of effects to test (could be got from AudMonkey in future)
sub getEffects{
...
# The duration setting does not work in AudMonkey 2.2.x where duration
# is read from pluginsettings.cfg
sub generateBase{
   my $genCmd = "Chirp";
   my $duration = 30.0;
