...
;;
;; Third party plug-ins are not translated by gettext in AudMonkey, but may include a
...
(defun aud-get-info (str)
  ...
  ;;; AudMonkey 2.3.0 does not fail if type is not recognised, it 
  ;;; falls back to a default, so test for valid types.
  ;;; 'Commands+' is not supported in AudMonkey 2.3.0
  (let (type
        info
