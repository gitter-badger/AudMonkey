...
static AudMonkeyProject::AttachedObjects::RegisteredFactory
...
ProjectAudioManager &ProjectAudioManager::Get( AudMonkeyProject &project )
{
...
ProjectAudioManager::ProjectAudioManager( AudMonkeyProject &project )
   : mProject{ project }
{
...
{
   AudMonkeyProject *project = &mProject;
   auto &projectAudioManager = *this;
   bool canStop = projectAudioManager.CanStopAudioStream();
...
WaveTrackArray ProjectAudioManager::ChooseExistingRecordingTracks(
   AudMonkeyProject &proj, bool selectedOnly, double targetRate)
{
   auto p = &proj;
...
void ProjectAudioManager::OnRecord(bool altAppearance)
{
   ...
   if (p) {
      ...
      if (!allSameRate) {
         AudMonkeyMessageBox(XO("The tracks selected "
         ...
         if (!existingTracks.empty()) {
            ...
            if (numberOfSelected > 0 && rateOfSelected != options.rate) {
               AudMonkeyMessageBox(XO(
                  ...
                  "(AudMonkey requires two channels at the same sample rate for\n"
                  "each stereo track)"),
...
bool ProjectAudioManager::DoRecord(AudMonkeyProject &project,
...
\n\
Other applications are competing with AudMonkey for processor time\n\
\n\
...
DefaultPlayOptions( AudMonkeyProject &project )
{
   auto &projectAudioIO = ProjectAudioIO::Get( project );
...
AudioIOStartStreamOptions
DefaultSpeedPlayOptions( AudMonkeyProject &project )
{
   auto &projectAudioIO = ProjectAudioIO::Get( project );
   auto gAudioIO = AudioIO::Get();
...
static RegisteredMenuItemEnabler stopIfPaused{{
   ...
   []( AudMonkeyProject &project, CommandFlag ){
...
GetPropertiesOfSelected(const AudMonkeyProject &proj)
{
