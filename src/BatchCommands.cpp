...
MacroCommands::MacroCommands( AudMonkeyProject &project )
: mProject{ project }
, mExporter{ project }
{
   ResetMacro();
...
MacroCommandsCatalog::MacroCommandsCatalog( const AudMonkeyProject *project )
{
   if (!project)
      return;
...
/// DoAudMonkeyCommand() takes a PluginID and executes the associated command.
///
/// At the moment flags are used only to indicate whether to prompt for
/// parameters
bool MacroCommands::DoAudMonkeyCommand(
...
//    SelectAllIfNone();
   ...
   EffectManager & em = EffectManager::Get();
   bool success = em.DoAudMonkeyCommand(ID, 
      context,
      &window,
   ...
   const CommandContext & Context)
{
   ...
   if( plug->GetPluginType() != PluginTypeAudMonkeyCommand )
   {
   ...
   const MacroCommandsCatalog &catalog, const wxString & filename)
{
   ...
   AudMonkeyProject *proj = &mProject;
   bool res = false;
...
void MacroCommands::MigrateLegacyChains()
{
   ...
   if (!done) {
      ...
      // an old AudMonkey version.  They will have their old chains intact, but
      // won't have any edits they made to the copy that now lives in Macros
      // which old AudMonkey will not read.
