...
#ifndef __AUDMONKEY_AUDIO_IO__
#define __AUDMONKEY_AUDIO_IO__
 ...
 * flexibility by PortAudio, but not used by AudMonkey - the data is stored in
 * the AudioIO class instead.
 */
int audmonkeyAudioCallback(
...
class AUDMONKEY_DLL_API AudioIoCallback /* not final */
   : public AudioIOBase
{
public:
   AudioIoCallback();
...
public:
   ...
   // Whether to check the error code passed to audmonkeyAudioCallback to
...
class AUDMONKEY_DLL_API AudioIO final
...
public:
   ...
   AudMonkeyProject *GetOwningProject() const { return mOwningProject; }
   ...
   bool IsAvailable(AudMonkeyProject *projecT) const;
   ...
   * audmonkey actually runs the audio I/O stream at. if there is no suitable
