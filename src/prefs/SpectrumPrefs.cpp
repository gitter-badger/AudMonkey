...
bool SpectrumPrefs::Validate()
{
   ...
   if (!mMaxFreq->GetValue().ToLong(&maxFreq)) {
      AudMonkeyMessageBox( XO("The maximum frequency must be an integer") );
      return false;
   }

   long minFreq;
   if (!mMinFreq->GetValue().ToLong(&minFreq)) {
      AudMonkeyMessageBox( XO("The minimum frequency must be an integer") );
      return false;
   }

   long gain;
   if (!mGain->GetValue().ToLong(&gain)) {
      AudMonkeyMessageBox( XO("The gain must be an integer") );
      return false;
   }

   long range;
   if (!mRange->GetValue().ToLong(&range)) {
      AudMonkeyMessageBox( XO("The range must be a positive integer") );
      return false;
   }

   long frequencygain;
   if (!mFrequencyGain->GetValue().ToLong(&frequencygain)) {
      AudMonkeyMessageBox( XO("The frequency gain must be an integer") );
      return false;
   }
   ...
   if (!mFindNotesMinA->GetValue().ToLong(&findNotesMinA)) {
      AudMonkeyMessageBox( XO("The minimum amplitude (dB) must be an integer") );
      return false;
   }

   long findNotesN;
   if (!mFindNotesN->GetValue().ToLong(&findNotesN)) {
      AudMonkeyMessageBox( XO("The maximum number of notes must be an integer") );
      return false;
   }
...
{
   return [=](wxWindow *parent, wxWindowID winid, AudMonkeyProject *pProject)
   {
