...
void DirectoriesPrefs::OnTempBrowse(wxCommandEvent &evt)
{
   ...
   if (retval != wxID_CANCEL && !dlog.GetPath().empty())
   {
      ...
      // Append an "audmonkey_temp" directory to this path if necessary (the
...
bool DirectoriesPrefs::Validate()
{
   ...
   if (Temp != oldDir) {
      AudMonkeyMessageBox(
         XO(
"Changes to temporary directory will not take effect until AudMonkey is restarted"),
...
{
   return [](wxWindow *parent, wxWindowID winid, AudMonkeyProject *)
   {
