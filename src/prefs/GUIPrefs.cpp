...
/* i18n-hint: refers to AudMonkey's user interface settings */
...
void GUIPrefs::Populate()
{
   ...
   Languages::GetLanguages(
      FileNames::AudMonkeyPathList(), mLangCodes, mLangNames);
   ...
   S.StartStatic(XO("Options"));
   {
      ...
      S.TieCheckBox(XXO("Re&tain labels if selection snaps to a label"),
                    {wxT("/GUI/RetainLabels"),
                     false});
      S.TieCheckBox(XXO("B&lend system and AudMonkey theme"),
                    {wxT("/GUI/BlendThemes"),
                     true});
#ifndef __WXMAC__
...
{
   auto result = Languages::SetLang(FileNames::AudMonkeyPathList(), lang);
