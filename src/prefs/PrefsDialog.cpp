...
// This isn't really used yet by wxWidgets as patched by AudMonkey for
// Mac accessibility, as of AudMonkey 2.3.2, but here it is anyway, keeping the
...
PrefsDialog::PrefsDialog(
   wxWindow * parent, AudMonkeyProject *pProject,
   const TranslatableString &titlePrefix,
   PrefsPanel::Factories &factories)
:  wxDialogWrapper(parent, wxID_ANY, XO("AudMonkey Preferences"),
            wxDefaultPosition,
            wxDefaultSize,
...
void PrefsDialog::OnOK(wxCommandEvent & WXUNUSED(event))
{
   ...
   if (gAudioIO) {
      ...
      // AudMonkeyProject::GetUpdateFlags().
   ...
   //      so AudMonkeyProject::UpdatePrefs() or any of the routines it calls must
...
void DoReloadPreferences( AudMonkeyProject &project )
{
