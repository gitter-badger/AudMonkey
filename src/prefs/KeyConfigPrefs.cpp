...
KeyConfigPrefs::KeyConfigPrefs(
   wxWindow * parent, wxWindowID winid, AudMonkeyProject *pProject,
...
{
   wxString file = wxT("AudMonkey-keys.xml");

   file = FileNames::SelectFile(FileNames::Operation::Open,
      XO("Select an XML file containing AudMonkey keyboard shortcuts..."),
      wxEmptyString,
      file,
      wxT(""),
...
"The file with the shortcuts contains illegal shortcut duplicates for \"%s\" and \"%s\".\nNothing is imported.")
   ...
   AudMonkeyMessageBox(message, XO("Loading Keyboard Shortcuts"), wxOK | wxCENTRE);
}
...
void KeyConfigPrefs::OnSet(wxCommandEvent & WXUNUSED(event))
{
   ...
   if (!oldCommands.empty()) {
      ...
      if (wxCANCEL == AudMonkeyMessageBox(
            XO(
