...
{
public:
   static ProjectManager &Get( AudMonkeyProject &project );
   static const ProjectManager &Get( const AudMonkeyProject &project );

   explicit ProjectManager( AudMonkeyProject &project );
   ...
   static void OpenFiles(AudMonkeyProject *proj);

   //! False when it is unsafe to overwrite proj with contents of an .aup3 file
   static bool SafeToOpenProjectInto(AudMonkeyProject &proj);
   ...
   public:
       ...
       */
      ProjectChooser( AudMonkeyProject *pProject, bool reuseNonemptyProject )
      ...
      AudMonkeyProject &operator() ( bool openingProjectFile );
      //! Commit the creation of any fresh project or changes to the existing project
      void Commit();

   private:
      AudMonkeyProject *mpGivenProject;
      AudMonkeyProject *mpUsedProject = nullptr;
      bool mReuseNonemptyProject;
   };

   //! Open a file into an AudMonkeyProject, returning the project, or nullptr for failure
   /*!
    ...
    @param fileNameArg path to the file to open; not always an AudMonkey project file, may be an import
    @param addtohistory whether to add .aup3 files to the MRU list (but always done for imports)
    @param reuseNonemptyProject if true, may reuse the given project when nonempty,
       but only if importing (not for a project file)
    */
   static AudMonkeyProject *OpenProject(
      AudMonkeyProject *pGivenProject,
