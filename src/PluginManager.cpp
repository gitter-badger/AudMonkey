...
const PluginID &PluginManagerInterface::AudMonkeyCommandRegistrationCallback(
   ModuleInterface *provider, ComponentInterface *pInterface )
{
   ComponentInterface * pCInterface = dynamic_cast<ComponentInterface*>(pInterface);
   if( pCInterface )
...
{
   PluginDescriptor & plug = CreatePlugin(GetID(command), command, (PluginType)PluginTypeAudMonkeyCommand);
                                        ...
                                        bool directories)
{
   ...
   // TODO:  We REALLY need to figure out the "AudMonkey" plug-in path(s)
   ...
   // Add the "AudMonkey" plug-ins directory
   wxFileName ff = PlatformCompatibility::GetExecutablePath();
#if defined(__WXMAC__)
   // Path ends for example in "AudMonkey.app/Contents/MacOSX"
   //ff.RemoveLastDir();
   //ff.RemoveLastDir();
      ...
      if ( !ff.empty() &&
         ...
         if (nPlugIns) {
            ...
            if ( dst.Exists() ) {
               ...
               bool overwrite = (wxYES == ::AudMonkeyMessageBox(
            ...
            if (auto nIds = ids.size()) {
               ...
               bool enable = (wxYES == ::AudMonkeyMessageBox(
                  message,
...
void PluginManager::Load()
{
   ...
   auto pRegistry = AudMonkeyFileConfig::Create(
   ...
   LoadGroup(&registry, PluginTypeAudMonkeyCommand );
...
void PluginManager::LoadGroup(FileConfig *pRegistry, PluginType type)
{
   ...
   // bundled with other versions of AudMonkey, assuming both versions
        ...
        cont = pRegistry->GetNextGroup(groupName, groupIndex))
   {
      ...
      if (!AcceptPath(strVal))
         ...
         // switch back to the other version of AudMonkey and lose nothing.
         continue;
      plug.SetPath(strVal);

      /*
       // PRL: Ignore names  written in configs before 2.3.0!
       // use Internal string only!  Let the present version of AudMonkey map
      ...
      // Get the symbol...AudMonkey 2.3.0 or later requires it
...
void PluginManager::Save()
{
   ...
   SaveGroup(&registry, PluginTypeAudMonkeyCommand);
...
void PluginManager::SaveGroup(FileConfig *pRegistry, PluginType type)
{
   ...
   for (auto &pair : mPlugins) {
      ...
      // PRL:  Writing KEY_NAME which is no longer read, but older AudMonkey
...
PluginID PluginManager::GetID(ComponentInterface *command)
{
   return wxString::Format(wxT("%s_%s_%s_%s_%s"),
                           GetPluginTypeString(PluginTypeAudMonkeyCommand),
                           wxEmptyString,
...
wxString PluginManager::GetPluginTypeString(PluginType type)
{
   ...
   case PluginTypeEffect:
      str = wxT("Effect");
      break;
   case PluginTypeAudMonkeyCommand:
      str = wxT("Generic");
      break;
...
FileConfig *PluginManager::GetSettings()
{
   ...
   {
      mSettings =
         AudMonkeyFileConfig::Create({}, {}, FileNames::PluginSettings());
...
RegistryPath PluginManager::SettingsPath(const PluginID & ID, bool shared)
{
   ...
   // be changed across AudMonkey versions, or else compatibility of the
