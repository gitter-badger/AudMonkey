...
//
ToolFrame::ToolFrame
   ( AudMonkeyProject *parent, ToolManager *manager, ToolBar *bar, wxPoint pos )
...
ToolManager &ToolManager::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< ToolManager >( key );
}

const ToolManager &ToolManager::Get( const AudMonkeyProject &project )
{
...
// Constructor
//
ToolManager::ToolManager( AudMonkeyProject *parent )
: wxEvtHandler()
{
   wxPoint pt[ 3 ];
...
void ToolManager::Destroy()
{
   if ( mTopDock || mBotDock ) { // destroy at most once
      ...
      // these toolbars. This change was needed to stop AudMonkey
...
// DA: Hides three more toolbars.
   ...
   // SetVUMeters(AudMonkeyProject *p);
   Updated();
}
...
void ToolManager::ReadConfig()
{
   ...
   // note that vMajor, vMinor, and vMicro will all be zero if either it's a new audmonkey.cfg file
   // or the version is less than 1.3.13 (when there were no version keys according to the comments in
   // InitPreferences()). So for new audmonkey.cfg
   ...
   for( ndx = 0; ndx < ToolBarCount; ndx++ )
   {
      ...
      if( dock )
      {
         ...
         // the space for the extra button is stored in audmonkey.cfg, and so you get an extra space
         // in the EditToolbar.
...
AttachedToolBarMenuItem::AttachedToolBarMenuItem(
   ...
   , mAttachedItem{
      ...
      (  MenuTable::FinderScope(
            [this](AudMonkeyProject &) -> CommandHandlerObject&
               { return *this; } ),
         MenuTable::Command( name, label_in,
            &AttachedToolBarMenuItem::OnShowToolBar,
            AlwaysEnabledFlag,
            CommandManager::Options{}.CheckTest( [id](AudMonkeyProject &project){
               auto &toolManager = ToolManager::Get( project );
