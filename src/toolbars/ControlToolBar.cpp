...
\brief A ToolBar that has the main Transport buttons.
  ...
  AudMonkey - they are not native controls on any platform -
...
static const TranslatableString
   /* i18n-hint: These are strings for the status bar, and indicate whether AudMonkey
...
// compatibility
ControlToolBar::ControlToolBar( AudMonkeyProject &project )
...
ControlToolBar *ControlToolBar::Find( AudMonkeyProject &project )
{
...
ControlToolBar &ControlToolBar::Get( AudMonkeyProject &project )
{
   auto &toolManager = ToolManager::Get( project );
   return *static_cast<ControlToolBar*>(
      toolManager.GetToolBar(TransportBarID) );
...
const ControlToolBar &ControlToolBar::Get( const AudMonkeyProject &project )
{
