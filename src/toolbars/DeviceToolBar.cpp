...
DeviceToolBar::DeviceToolBar( AudMonkeyProject &project )
...
DeviceToolBar &DeviceToolBar::Get( AudMonkeyProject &project )
{
   auto &toolManager = ToolManager::Get( project );
   return *static_cast<DeviceToolBar*>( toolManager.GetToolBar(DeviceBarID) );
}

const DeviceToolBar &DeviceToolBar::Get( const AudMonkeyProject &project )
{
...
void DeviceToolBar::ShowComboDialog(wxChoice *combo, const TranslatableString &title)
{
   if (!combo || combo->GetCount() == 0) {
      AudMonkeyMessageBox( XO("Device information is not available.") );
      return;
   }
