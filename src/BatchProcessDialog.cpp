...
ApplyMacroDialog::ApplyMacroDialog(
   wxWindow * parent, AudMonkeyProject &project, bool bInherited):
...
void ApplyMacroDialog::OnApplyToProject(wxCommandEvent & WXUNUSED(event))
{
   ...
   if (item == -1) {
      AudMonkeyMessageBox(XO("No macro selected"));
      return;
   }
...
MacrosWindow::MacrosWindow(
   wxWindow * parent, AudMonkeyProject &project, bool bExpanded):
...
void MacrosWindow::OnAdd(wxCommandEvent & WXUNUSED(event))
{
   ...
   while (true) {
      AudMonkeyTextEntryDialog d(this,
...
/// Send changed values back to Prefs, and update AudMonkey.
