...
bool LabelTrackView::CalcCursorX( AudMonkeyProject &project, int * x) const
{
...
bool LabelTrackView::IsTextSelected( AudMonkeyProject &project ) const
{
   if ( !HasSelection( project ) )
      return false;
...
bool LabelTrackView::CutSelectedText( AudMonkeyProject &project )
{
...
///  @return true if text is selected in text box, false otherwise
bool LabelTrackView::CopySelectedText( AudMonkeyProject &project )
{
   if ( !HasSelection( project ) )
      return false;
...
int LabelTrackView::GetSelectedIndex( AudMonkeyProject &project ) const
{
...
bool LabelTrackView::DoCaptureKey(
   AudMonkeyProject &project, wxKeyEvent & event )
{
...
unsigned LabelTrackView::CaptureKey(
   wxKeyEvent & event, ViewInfo &, wxWindow *, AudMonkeyProject *project )
{
...
unsigned LabelTrackView::Char(
   wxKeyEvent & event, ViewInfo &viewInfo, wxWindow *, AudMonkeyProject *project)
{
   double bkpSel0 = viewInfo.selectedRegion.t0(),
      bkpSel1 = viewInfo.selectedRegion.t1();
   // Pass keystroke to labeltrack's handler and add to history if any
...
bool LabelTrackView::DoKeyDown(
   AudMonkeyProject &project, NotifyingSelectedRegion &newSel, wxKeyEvent & event)
{
...
bool LabelTrackView::DoChar(
   AudMonkeyProject &project, NotifyingSelectedRegion &WXUNUSED(newSel),
   wxKeyEvent & event)
{
   // Check for modifiers and only allow shift.
   //
...
void LabelTrackView::ShowContextMenu( AudMonkeyProject &project )
{
...
void LabelTrackView::OnContextMenu(
   AudMonkeyProject &project, wxCommandEvent & evt )
{
   auto &selectedRegion = ViewInfo::Get( project ).selectedRegion;
...
bool LabelTrackView::HasSelection( AudMonkeyProject &project ) const
{
...
(AudMonkeyProject &project, LabelTrack *lt, int index)
{
   ...
   const SelectedRegion& region, const wxString& initialValue, wxString& value)
{
   ...
   AudMonkeyTextEntryDialog dialog{ &window,
      XO("Name:"),
      XO("New label"),
      initialValue,
      wxOK | wxCANCEL,
   ...
   // keep the dialog within AudMonkey's window, so that the dialog is always fully visible
