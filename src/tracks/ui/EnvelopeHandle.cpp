...
void EnvelopeHandle::Enter(bool, AudMonkeyProject *)
{
...
namespace {
   void GetTimeTrackData
      (const AudMonkeyProject &project, const TimeTrack &tt,
...
(std::weak_ptr<EnvelopeHandle> &holder,
 const wxMouseState &state, const wxRect &rect, const AudMonkeyProject *pProject,
...
(const TrackPanelMouseState &, AudMonkeyProject *pProject)
{
   const bool unsafe = ProjectAudioIO::Get( *pProject ).IsAudioActive();
 ...
 wxWindow *)
{
   ...
   ProjectHistory::Get( *pProject ).PushState(
      /* i18n-hint: (verb) AudMonkey has just adjusted the envelope .*/
...
UIHandle::Result EnvelopeHandle::Cancel(AudMonkeyProject *pProject)
{
