...
\class Track
\brief Fundamental data object of AudMonkey, displayed in the TrackPanel.
...
static const AudMonkeyProject::AttachedObjects::RegisteredFactory key{
   [](AudMonkeyProject &project) { return TrackList::Create( &project ); }
};

TrackList &TrackList::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< TrackList >( key );
}

const TrackList &TrackList::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

TrackList::TrackList( AudMonkeyProject *pOwner )
:  wxEvtHandler()
, mOwner{ pOwner }
{
}
...
// Factory function
std::shared_ptr<TrackList> TrackList::Create( AudMonkeyProject *pOwner )
{
