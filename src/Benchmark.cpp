...
public:
   ...
   BenchmarkDialog( wxWindow *parent, AudMonkeyProject &project );
...
BenchmarkDialog::BenchmarkDialog(
   wxWindow *parent, AudMonkeyProject &project)
   :
...
void BenchmarkDialog::OnRun( wxCommandEvent & WXUNUSED(event))
{
   ...
   // This code will become part of libaudmonkey,
   ...
   for (z = 0; z < trials; z++) {
      ...
      catch (const AudMonkeyException&) {
         Printf( XO("Trial %d\n").Format( z ) );
   ...
   system("ls .audmonkey_temp/* | wc --lines");
#endif
