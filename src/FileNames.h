...
#ifndef __AUDMONKEY_FILE_NAMES__
#define __AUDMONKEY_FILE_NAMES__
...
namespace FileNames
{
   ...
   extern AUDMONKEY_DLL_API const FileType
        AllFiles // *
      , AudMonkeyProjects // *.aup3
   ...
   // selection dialog
   AUDMONKEY_DLL_API wxString FormatWildcard( const FileTypes &fileTypes );

   // This exists to compensate for bugs in wxCopyFile:
   AUDMONKEY_DLL_API bool DoCopyFile(
   ...
   AUDMONKEY_DLL_API wxString MkDir(const wxString &Str);

   AUDMONKEY_DLL_API bool IsMidi(const FilePath &fName);

   /** \brief A list of directories that should be searched for AudMonkey files
    * (plug-ins, help files, etc.).
    *
    * On Unix this will include the directory AudMonkey was installed into,
    * plus the current user's .audmonkey-data/Plug-Ins directory.  Additional
    * directories can be specified using the AUDMONKEY_PATH environment
    * variable.  On Windows or Mac OS, this will include the directory
    * which contains the AudMonkey program. */
   AUDMONKEY_DLL_API const FilePaths &AudMonkeyPathList();
   AUDMONKEY_DLL_API void SetAudMonkeyPathList( FilePaths list );

   // originally an ExportMultipleDialog method. Append suffix if newName appears in otherNames.
   AUDMONKEY_DLL_API void MakeNameUnique(
      FilePaths &otherNames, wxFileName &newName);

   AUDMONKEY_DLL_API wxString LowerCaseAppNameInPath( const wxString & dirIn);
   /** \brief AudMonkey user data directory
    *
    * Where audmonkey keeps its settings and other user data squirreled away,
    * by default ~/.audmonkey-data/ on Unix, Application Data/AudMonkey on
    * windows system */
   AUDMONKEY_DLL_API FilePath DataDir();
   AUDMONKEY_DLL_API FilePath ResourcesDir();
   AUDMONKEY_DLL_API FilePath HtmlHelpDir();
   AUDMONKEY_DLL_API FilePath HtmlHelpIndexFile(bool quick);
   AUDMONKEY_DLL_API FilePath LegacyChainDir();
   AUDMONKEY_DLL_API FilePath MacroDir();
   AUDMONKEY_DLL_API FilePath NRPDir();
   AUDMONKEY_DLL_API FilePath NRPFile();
   AUDMONKEY_DLL_API FilePath PluginRegistry();
   AUDMONKEY_DLL_API FilePath PluginSettings();

   AUDMONKEY_DLL_API FilePath BaseDir();
   AUDMONKEY_DLL_API FilePath ModulesDir();
    ...
    * <DataDir>/Plug-Ins/ */
   AUDMONKEY_DLL_API FilePath PlugInDir();
   AUDMONKEY_DLL_API FilePath ThemeDir();
   AUDMONKEY_DLL_API FilePath ThemeComponentsDir();
   AUDMONKEY_DLL_API FilePath ThemeCachePng();
   AUDMONKEY_DLL_API FilePath ThemeCacheAsCee();
   AUDMONKEY_DLL_API FilePath ThemeComponent(const wxString &Str);
   AUDMONKEY_DLL_API FilePath ThemeCacheHtm();
   AUDMONKEY_DLL_API FilePath ThemeImageDefsAsCee();

   // Obtain name of loaded module that contains address
   AUDMONKEY_DLL_API FilePath PathFromAddr(void *addr);

   AUDMONKEY_DLL_API bool IsPathAvailable( const FilePath & Path);
   AUDMONKEY_DLL_API wxFileNameWrapper DefaultToDocumentsFolder
   ...
   AUDMONKEY_DLL_API wxString PreferenceKey(FileNames::Operation op, FileNames::PathType type);

   AUDMONKEY_DLL_API FilePath FindDefaultPath(Operation op);
   AUDMONKEY_DLL_API void UpdateDefaultPath(Operation op, const FilePath &path);
   ...
   AUDMONKEY_DLL_API void AddUniquePathToPathList(const FilePath &path,
                                       FilePaths &pathList);
   AUDMONKEY_DLL_API void AddMultiPathsToPathList(const wxString &multiPathString,
                                       FilePaths &pathList);
   AUDMONKEY_DLL_API void FindFilesInPathList(const wxString & pattern,
   ...
   AUDMONKEY_DLL_API char *VerifyFilename(const wxString &s, bool input = true);
#endif

   //! Check location on writable access and return true if checked successfully.
   AUDMONKEY_DLL_API bool WritableLocationCheck(const FilePath& path);

   // wxString compare function for sorting case, which is needed to load correctly.
   AUDMONKEY_DLL_API int CompareNoCase(const wxString& first, const wxString& second);

   // Create a unique filename using the passed prefix and suffix
   AUDMONKEY_DLL_API wxString CreateUniqueName(const wxString &prefix,
                             const wxString &suffix = wxEmptyString);

   // File extension used for unsaved/temporary project files
   AUDMONKEY_DLL_API wxString UnsavedProjectExtension();
