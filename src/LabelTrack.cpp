...
Track::Holder LabelTrack::PasteInto( AudMonkeyProject & ) const
{
...
LabelStruct LabelStruct::Import(wxTextFile &file, int &index)
{
   ...
   // audmonkey will ignore.  Test for the presence of such a line and then
   // parse it if we can.
...
void LabelStruct::Export(wxTextFile &file) const
{
   ...
   // so that earlier versions of AudMonkey ignore it.
...
void LabelTrack::Import(wxTextFile & in)
{
   ...
   if (error)
      ::AudMonkeyMessageBox( XO("One or more saved labels could not be read.") );
   SortLabels();
}
...
bool LabelTrack::HandleXMLTag(const wxChar *tag, const wxChar **attrs)
{
   if (!wxStrcmp(tag, wxT("label"))) {
      ...
      // Handle files created by AudMonkey 1.1.   Labels in AudMonkey 1.1
