...
unsigned operator()
   ( const TrackPanelMouseEvent &evt, AudMonkeyProject *pProject ) const
{
   ...
   else if (event.CmdDown())
   {
         ...
         // using AudMonkeyProject zooming, which is smarter,
...
AudMonkeyProject::AttachedWindows::RegisteredFactory sProjectWindowKey{
   []( AudMonkeyProject &parent ) -> wxWeakRef< wxWindow > {
      wxRect wndRect;
...
ProjectWindow &ProjectWindow::Get( AudMonkeyProject &project )
{
   return project.AttachedWindows::Get< ProjectWindow >( sProjectWindowKey );
}

const ProjectWindow &ProjectWindow::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

ProjectWindow *ProjectWindow::Find( AudMonkeyProject *pProject )
{
   return pProject
...
const ProjectWindow *ProjectWindow::Find( const AudMonkeyProject *pProject )
{
   return Find( const_cast< AudMonkeyProject * >( pProject ) );
}
...
//yet checked in to AudMonkey SVN as of 12-Feb-2010
...
ProjectWindow::ProjectWindow(wxWindow * parent, wxWindowID id,
                                 const wxPoint & pos,
                                 const wxSize & size, AudMonkeyProject &project)
   : ProjectWindowBase{ parent, id, pos, size, project }
{
   ...
   /* i18n-hint: This is an experimental feature where the main panel in
      AudMonkey is put on a notebook tab, and this is the name on that tab.
   ...
   // flicker when AudMonkey starts up.
...
void ProjectWindow::FixScrollbars()
{
   ...
   // of an AudMonkeyProject reopened from disk.
...
void ProjectWindow::OnShow(wxShowEvent & event)
{
   ...
   // (From Debian...see also TrackPanel::OnTimer and AudMonkeyTimer::Notify)
   //
   ...
   //  operation, causing reentry. AudMonkey had a workaround in place
...
void ProjectWindow::OnActivate(wxActivateEvent & event)
{
   ...
   // AudMonkey from a different application.
   //
   // This was observed by minimizing all windows using WINDOWS+M and
   // then ALT+TAB to return to AudMonkey.  Focus will be given to the
...
ProjectWindow::PlaybackScroller::PlaybackScroller(AudMonkeyProject *project)
: mProject(project)
{
