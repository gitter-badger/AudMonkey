/*!********************************************************************
  ...
  @file AudMonkeyException.cpp
  @brief Implements AudMonkeyException and related
...
AudMonkeyException::~AudMonkeyException()
{
}
   ...
   if (!moved) {
      ...
      if ( wxAtomicDec( sOutstandingMessages ) == 0 ) {
         if (exceptionType == ExceptionType::Internal)
         {
            ShowExceptionDialog(
               nullptr,
               (caption.empty() ? AudMonkeyMessageBoxCaptionStr() : caption),
