...
#ifndef __AUDMONKEY_PITCHNAME__
#define __AUDMONKEY_PITCHNAME__
...
AUDMONKEY_DLL_API double FreqToMIDInote(const double freq);

AUDMONKEY_DLL_API double MIDInoteToFreq(const double dMIDInote);
...
AUDMONKEY_DLL_API unsigned int PitchIndex(const double dMIDInote);

// PitchOctave returns the octave index for a double dMIDInote note number,
// per result from FreqToMIDInote.
// MIDI note number 0 is C-1 in Scientific pitch notation.
AUDMONKEY_DLL_API int PitchOctave(const double dMIDInote);
...
AUDMONKEY_DLL_API TranslatableString PitchName(
...
AUDMONKEY_DLL_API TranslatableString PitchName_Absolute(
   const double dMIDInote,
   const PitchNameChoice choice = PitchNameChoice::Sharps);
