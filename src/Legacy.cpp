...
\file Legacy.cpp
\brief Converts old AudMonkey file types.  Implements
AutoRollbackRenamer.

  These routines convert AudMonkey project files from the
  0.98...1.0 format into an XML format that's compatible with
  AudMonkey 1.2.0 and newer.
...
bool ConvertLegacyProjectFile(const wxFileName &filename)
{
   ...
   return GuardedCall< bool >( [&] {
      ...
      if (f.GetFirstLine() != wxT("AudMonkeyProject"))
         return false;
      ...
      xmlFile.EndTag(wxT("audmonkeyproject"));
      xmlFile.Commit();
