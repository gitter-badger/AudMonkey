...
MenuManager &MenuManager::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< MenuManager >( key );
}

const MenuManager &MenuManager::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

MenuManager::MenuManager( AudMonkeyProject &project )
   : mProject{ project }
{
   UpdatePrefs();
...
#else
   // AudMonkey autoselects or warns.
...
{
   MenuItemVisitor( AudMonkeyProject &proj, CommandManager &man )
...
void MenuCreator::CreateMenusAndCommands(AudMonkeyProject &project)
{
...
void MenuManager::ModifyUndoMenuItems(AudMonkeyProject &project)
{
...
void MenuCreator::RebuildMenuBar(AudMonkeyProject &project)
{
...
void MenuManager::ModifyToolbarMenus(AudMonkeyProject &project)
{
...
void MenuCreator::RebuildAllMenuBars()
{
   for( auto p : AllProjects{} ) {
      ...
      //
      // This workaround should be removed when AudMonkey updates to wxWidgets 3.x which has a fix.
