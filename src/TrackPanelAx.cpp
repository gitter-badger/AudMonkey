...
TrackPanelAx::TrackPanelAx( AudMonkeyProject &project )
   :
...
wxAccStatus TrackPanelAx::GetLocation( wxRect& rect, int elementId )
{
   ...
   else
   {
      ...
      // Inflate the screen reader's rectangle so it overpaints AudMonkey's own
...
TrackFocus &TrackFocus::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< TrackFocus >( key );
}

const TrackFocus &TrackFocus::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

TrackFocus::TrackFocus( AudMonkeyProject &project )
   : mProject{ project }
{
}
