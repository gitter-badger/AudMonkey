...
bool EffectManager::DoAudMonkeyCommand(const PluginID & ID,
...

{
   ...
   AudMonkeyCommand *command = GetAudMonkeyCommand(ID);
   ...
   bool res = command->DoAudMonkeyCommand(parent, context, shouldPrompt);
...
ManualPageID EffectManager::GetCommandUrl(const PluginID & ID)
{
   ...
   AudMonkeyCommand * pCom = GetAudMonkeyCommand(ID);
   if( pCom )
...
void EffectManager::GetCommandDefinition(const PluginID & ID, const CommandContext & context, int flags)
{
   ...
   if( !command )
      command = GetAudMonkeyCommand( ID );
   if( !command )
      return;
...
Effect *EffectManager::GetEffect(const PluginID & ID)
{
   ...
   if (mEffects.find(ID) == mEffects.end())
   {
      ...
      auto command = dynamic_cast<AudMonkeyCommand *>(PluginManager::Get().GetInstance(ID));
      if( !command )
         AudMonkeyMessageBox(
            XO(
...
AudMonkeyCommand *EffectManager::GetAudMonkeyCommand(const PluginID & ID)
{
   // Must have a "valid" ID
   if (ID.empty())
   {
   ...
   if (mCommands.find(ID) == mCommands.end())
   {
      ...
      if (ident && ident->IsLegacy())
      {
         auto command = dynamic_cast<AudMonkeyCommand *>(ident);
      ...
      auto command = std::make_shared<AudMonkeyCommand>(); // TODO: use make_unique and store in std::unordered_map
      if (command)
      {
         AudMonkeyCommand *client = dynamic_cast<AudMonkeyCommand *>(ident);
...
const PluginID & EffectManager::GetEffectByIdentifier(const CommandID & strTarget)
{
   ...
   for (auto &plug
        : pm.PluginsOfType(PluginTypeEffect | PluginTypeAudMonkeyCommand)) {
      auto &ID = plug.GetID();
