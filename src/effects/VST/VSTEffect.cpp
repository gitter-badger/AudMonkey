...
// TODO:  Unfortunately we have some dependencies on AudMonkey provided 
...
//
// This is the symbol that AudMonkey looks for when the module is built as a
// dynamic library.
//
// When the module is builtin to AudMonkey, we use the same function, but it is
// declared static so as not to clash with other builtin modules.
//
// ============================================================================
DECLARE_MODULE_ENTRY(AudMonkeyModule)
{
...
// the wxApp::OnInit() method is called.  We check to see if AudMonkey was
...
public:
   bool OnInit()
   {
      ...
      if (wxTheApp && wxTheApp->argc == 3 && wxStrcmp(wxTheApp->argv[1], VSTCMDKEY) == 0)
      {
         ...
         //        hide AudMonkey failures in the subprocess, so if you're having an unruley
         //        VST or odd AudMonkey failures, comment it out and you might get more info.
...
{
   return XO("Adds the ability to use VST effects in AudMonkey.");
}
...
// static
//
// Called from reinvokation of AudMonkey or DLL to check in a separate process
                                ...
                                float opt)
{
   ...
   switch (opcode)
   {
      ...
      case audioMasterGetVendorString:
         strcpy((char *) ptr, "AudMonkey Team");    // Do not translate, max 64 + 1 for null terminator
         return 1;

      case audioMasterGetProductString:
         strcpy((char *) ptr, "AudMonkey");         // Do not translate, max 64 + 1 for null terminator
         return 1;
...
///
/// And at the same time I added buffer delay compensation, which allows AudMonkey
...
void VSTEffect::ExportPresets()
{
   ...
   path = FileNames::SelectFile(FileNames::Operation::Presets,
      ...
      wxT("xml"),
      {
        ...
        { XO("AudMonkey VST preset file"), { wxT("xml") }, true },
      },
...
bool VSTEffect::Load()
{
   ...
   // others were hit or miss.  The cause was that we export all of AudMonkey's
   // symbols and some of the loaded libraries were picking up AudMonkey's and 
   // not their own.
   //
   ...
   // and that "basically" puts AudMonkey last when the loader needs to resolve
   // symbols.
   //
...
#define RTLD_DEEPBIND 0
   ...
   if (mAEffect)
   {
         ...
         mAEffect->flags & effFlagsCanReplacing)
      {
         ...
         // crash AudMonkey when saving the initial default parameters
         // with this.
         callSetProgram(0);
