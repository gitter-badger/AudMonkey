...
\class Effect
\brief Base class for many of the effects in AudMonkey.
...
EffectFamilySymbol Effect::GetFamily()
{
   ...
   // effect family.
   return { wxT("AudMonkey"), XO("Built-in") };
}
                      ...
                      const EffectDialogFactory &dialogFactory)
{
   ...
   auto &pProject = *const_cast<AudMonkeyProject*>(FindProject()); // how to remove this const_cast?
...
bool Effect::Process()
{
   ...
   // the parameters (the AudMonkey Reverb effect does when the stereo width is 0).
   mNumAudioIn = GetAudioInCount();
   mNumAudioOut = GetAudioOutCount();
                          ...
                          ArrayOf< float *> &outBufPos)
{
   ...
   while (inputRemaining != 0 || delayRemaining != 0)
   {
      ...
      catch( const AudMonkeyException & WXUNUSED(e) )
      {
         // PRL: Bug 437:
...
void Effect::CopyInputTracks(bool allSyncLockSelected)
{
   ...
   mOutputTracks = TrackList::Create(
      const_cast<AudMonkeyProject*>( FindProject() ) // how to remove this const_cast?
  );
...
const AudMonkeyProject *Effect::FindProject() const
{
   if (!inputTracks())
      return nullptr;
