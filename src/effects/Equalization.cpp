...
*//****************************************************************//**
   ...
   Clone of the FFT Filter effect, no longer part of AudMonkey.
...
//
// Update presets to match AudMonkey version.
//
...
bool EffectEqualization::GetDefaultFileName(wxFileName &fileName)
{
   ...
   if( !fileName.FileExists() )
   {
      ...
      //   errorMessage, wxT("http://wiki.audmonkeyteam.org/wiki/EQCurvesDownload"), false);
...
void EditCurvesDialog::OnRename(wxCommandEvent & WXUNUSED(event))
{
   ...
   while(item >= 0)
   {
      ...
      {   // Show the dialog and bail if the user cancels
         bad = false;
         // build the dialog
         AudMonkeyTextEntryDialog dlg( this,
...
{
   // full path to wiki.
   wxLaunchDefaultBrowser(wxT("https://wiki.audmonkeyteam.org/wiki/EQCurvesDownload"));
}
