...
#ifndef __AUDMONKEY_UNDOMANAGER__
#define __AUDMONKEY_UNDOMANAGER__
...
// Events emitted by AudMonkeyProject for the use of listeners

// Project state did not change, but a new state was copied into Undo history
// and any redo states were lost
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API, EVT_UNDO_PUSHED, wxCommandEvent);

// Project state did not change, but current state was modified in Undo history
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API, EVT_UNDO_MODIFIED, wxCommandEvent);

// Project state did not change, but current state was renamed in Undo history
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API, EVT_UNDO_RENAMED, wxCommandEvent);
...
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API, EVT_UNDO_OR_REDO, wxCommandEvent);

// Project state changed other than for single-step undo/redo; undo manager
// contents did not change other than the pointer to current state
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API, EVT_UNDO_RESET, wxCommandEvent);

// Undo or redo states discarded
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API, EVT_UNDO_PURGE, wxCommandEvent);
...
class AUDMONKEY_DLL_API UndoManager final
   : public ClientData::Base
{
 public:
   static UndoManager &Get( AudMonkeyProject &project );
   static const UndoManager &Get( const AudMonkeyProject &project );
 
   explicit
   UndoManager( AudMonkeyProject &project );
   ~UndoManager();
