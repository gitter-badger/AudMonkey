...
UpdatePopupDialog::UpdatePopupDialog (wxWindow* parent, const VersionPatch& versionPatch)
    : wxDialogWrapper (parent, -1, XC("Update AudMonkey", "update dialog"),
...
HtmlWindow* UpdatePopupDialog::AddHtmlContent (wxWindow* parent)
{
    ...
    informationStr
        ...
        << XC("AudMonkey %s is available!", "update dialog").Format(mVersionPatch.version.GetString()).Translation()
        << wxT("</h3><h5>")
    ...
    informationStr << wxT("<a href = \"https://github.com/audmonkey/audmonkey/releases\">");
