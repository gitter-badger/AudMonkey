...
wxString ChooseLanguage(wxWindow *parent)
{
    ...
    * time AudMonkey has been run. */
   LangChoiceDialog dlog(parent, -1, XO("AudMonkey First Run"));
   dlog.CentreOnParent();
   dlog.ShowModal();
...
{
   SetName();
   const auto &paths = FileNames::AudMonkeyPathList();
   ...
   S.StartVerticalLay(false);
   {
      ...
      {
         S.SetBorder(15);
         mChoice = S.AddChoice(XXO("Choose Language for AudMonkey to use:"),
            mLangNames,
            lang);
      }
      S.EndVerticalLay();
   ...
   if (mLang.Left(2) != slang.Left(2)) {
      ...
      auto msg = XO("The language you have chosen, %s (%s), is not the same as the system language, %s (%s).")
         .Format(mLangNames[ndx],
                 ...
                 sname,
                 slang);
      if ( wxNO == AudMonkeyMessageBox( msg, XO("Confirm"), wxYES_NO ) ) {
         return;
      }
   }
