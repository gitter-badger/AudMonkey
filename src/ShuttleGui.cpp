...
\class ShuttleGui
\brief
  Derived from ShuttleGuiBase, an AudMonkey specific class for shuttling
  ...
  ShuttleGuiBase.
     ...
     - AudMonkey specific widgets are dealt with by ShuttleGui
   ...
   std::initializer_list<const ListControlColumn> columns )
{
   ...
   // columns are deleted and later InsertItem()s will cause AudMonkey to crash.
...
// In programs like AudMonkey which don't use internal program variables
...
// Now we have AudMonkey specific shuttle functions.
