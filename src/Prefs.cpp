...
\brief Utility functions for working with our wxConf (gPrefs)
  ...
  AudMonkey uses wxWidgets' wxConfig class to handle preferences.
...
\verbatim
  ...
  Preference field specification:
   /
      Version					- AudMonkey Version that created these prefs
...
void EnumSettingBase::Migrate( wxString &value )
{
   ...
   if ( !mOldKey.empty() &&
      ...
      // AudMonkey.  But further changes will be stored only to the NEW key
