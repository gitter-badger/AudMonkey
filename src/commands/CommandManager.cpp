...
CommandManager &CommandManager::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< CommandManager >( key );
}

const CommandManager &CommandManager::Get( const AudMonkeyProject &project )
{
...
void CommandManager::UpdateCheckmarks( AudMonkeyProject &project )
{
...
void CommandManager::AddItem(AudMonkeyProject &project,
...
{
   return [=](AudMonkeyProject&){ return gPrefs->ReadBool( key, defaultValue ); };
}
...
///
///
bool CommandManager::FilterKeyEvent(AudMonkeyProject *project, const wxKeyEvent & evt, bool permit)
{
   if (!project)
      return false;
...
bool CommandManager::HandleCommandEntry(AudMonkeyProject &project,
...
bool CommandManager::HandleMenuID(
   AudMonkeyProject &project, int id, CommandFlag flags, bool alwaysEnabled)
{
   mLastProcessId = id;
   CommandListEntry *entry = mCommandNumericIDHash[id];
...
TranslatableStrings CommandManager::GetCategories( AudMonkeyProject& )
{
...
{
   if (!wxStrcmp(tag, wxT("audmonkeykeyboard"))) {
      mXMLKeysRead = 0;
   }
...
// may throw
{
   ...
   xmlFile.EndTag(wxT("audmonkeykeyboard"));
}
...
// of these is changed, then if /GUI/Shortcuts/FullDefaults is not set in audmonkey.cfg,
// because the defaults appear as user assigned shortcuts in audmonkey.cfg,
...
void CommandManager::RemoveDuplicateShortcuts()
{
   ...
   if (!disabledShortcuts.Translation().empty()) {
      ...
      " that you have assigned to another command.")
         + disabledShortcuts;
      AudMonkeyMessageBox(message, XO("Shortcuts have been removed"), wxOK | wxCENTRE);
