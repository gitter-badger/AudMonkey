...
{
   return XO("Provides builtin commands to AudMonkey");
}
   ...
   for (const auto &pair : mCommands)
   {
      ...
      if (!pm.IsPluginRegistered(path, &pair.second->name.Msgid()))
      {
         ...
         DiscoverPluginsAtPath(path, ignoredErrMsg,
            PluginManagerInterface::AudMonkeyCommandRegistrationCallback);
      }
   }
...
std::unique_ptr<AudMonkeyCommand> BuiltinCommandsModule::Instantiate(const PluginPath & path)
{
