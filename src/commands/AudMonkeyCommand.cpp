...
\brief Base class for command in AudMonkey.

*//****************************************************************//**

\class AudMonkeyCommandDialog
...
AudMonkeyCommand::VetoDialogHook &GetVetoDialogHook()
{
   static AudMonkeyCommand::VetoDialogHook sHook = nullptr;
   return sHook;
}

}

auto AudMonkeyCommand::SetVetoDialogHook( VetoDialogHook hook )
   -> VetoDialogHook
{
...
AudMonkeyCommand::AudMonkeyCommand()
{
   mProgress = NULL;
   mUIParent = NULL;
   mUIDialog = NULL;
...
bool AudMonkeyCommand::ShowInterface(wxWindow *parent, bool WXUNUSED(forceModal))
{
   if (mUIDialog)
   {
...
wxDialog *AudMonkeyCommand::CreateUI(wxWindow *parent, AudMonkeyCommand * WXUNUSED(client))
{
   Destroy_ptr<AudMonkeyCommandDialog> dlg { safenew AudMonkeyCommandDialog{
      parent, GetName(), this}};
...
bool AudMonkeyCommand::GetAutomationParameters(wxString & parms)
{
...
bool AudMonkeyCommand::SetAutomationParameters(const wxString & parms)
{
...
bool AudMonkeyCommand::DoAudMonkeyCommand(wxWindow *parent,
                      ...
                      bool shouldPrompt /* = true */)
{
   ...
   // Prompting may call AudMonkeyCommand::Preview
   ...
   bool returnVal = true;
   bool skipFlag = CheckWhetherSkipAudMonkeyCommand();
   if (skipFlag == false)
   {
      auto name = GetName();
...
bool AudMonkeyCommand::PromptUser(wxWindow *parent)
{
   return ShowInterface(parent, IsBatchProcessing());
}

bool AudMonkeyCommand::TransferDataToWindow()
{
...
bool AudMonkeyCommand::TransferDataFromWindow()
{
...
int AudMonkeyCommand::MessageBox(
   ...
   const TranslatableString &titleStr)
{
   ...
   return AudMonkeyMessageBox(message, title, style, mUIParent);
}

BEGIN_EVENT_TABLE(AudMonkeyCommandDialog, wxDialogWrapper)
   EVT_BUTTON(wxID_OK, AudMonkeyCommandDialog::OnOk)
   EVT_BUTTON(wxID_HELP, AudMonkeyCommandDialog::OnHelp)
   EVT_BUTTON(wxID_CANCEL, AudMonkeyCommandDialog::OnCancel)
END_EVENT_TABLE()

AudMonkeyCommandDialog::AudMonkeyCommandDialog(wxWindow * parent,
                           const TranslatableString & title,
                           AudMonkeyCommand * pCommand,
                           int type,
                           int flags,
...
bool AudMonkeyCommandDialog::Init()
{
...
/// kind of dialog.
void AudMonkeyCommandDialog::PopulateOrExchange(ShuttleGui & S)
{
   wxASSERT( mpCommand );
   mpCommand->PopulateOrExchange( S );
}

bool AudMonkeyCommandDialog::TransferDataToWindow()
{
...
bool AudMonkeyCommandDialog::TransferDataFromWindow()
{
...
bool AudMonkeyCommandDialog::Validate()
{
   return true;
}

void AudMonkeyCommandDialog::OnOk(wxCommandEvent & WXUNUSED(evt))
{
   ...
   // just our AudMonkeyCommands dialogs.  So, this is a only temporary workaround
...
void AudMonkeyCommandDialog::OnCancel(wxCommandEvent & WXUNUSED(evt))
{
   EndModal(false);
}

void AudMonkeyCommandDialog::OnHelp(wxCommandEvent & WXUNUSED(event))
{
   if( mpCommand )
   {
