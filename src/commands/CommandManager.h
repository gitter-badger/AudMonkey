...
#ifndef __AUDMONKEY_COMMAND_MANAGER__
#define __AUDMONKEY_COMMAND_MANAGER__
...
class AUDMONKEY_DLL_API CommandManager final
   : public XMLTagHandler
   , public ClientData::Base
{
 public:
   static CommandManager &Get( AudMonkeyProject &project );
   static const CommandManager &Get( const AudMonkeyProject &project );
   ...
   using CheckFn = std::function< bool(AudMonkeyProject&) >;

   // For specifying unusual arguments in AddItem
   struct AUDMONKEY_DLL_API Options
   {
      Options() {}
   ...
   void AddItem(AudMonkeyProject &project,
   ...
   bool FilterKeyEvent(AudMonkeyProject *project, const wxKeyEvent & evt, bool permit = false);
   bool HandleMenuID(AudMonkeyProject &project, int id, CommandFlag flags, bool alwaysEnabled);
   ...
   TranslatableStrings GetCategories( AudMonkeyProject& );
...
private:
   ...
   bool HandleCommandEntry(AudMonkeyProject &project,
...
public:
   ...
   void UpdateCheckmarks( AudMonkeyProject &project );
private:
...
struct AUDMONKEY_DLL_API MenuVisitor : Registry::Visitor
{
   // final overrides
...
{
   explicit ToolbarMenuVisitor( AudMonkeyProject &p ) : project{ p } {}
   operator AudMonkeyProject & () const { return project; }
...
namespace MenuTable {
   ...
   struct AUDMONKEY_DLL_API MenuSection {
      virtual ~MenuSection();
   };
   struct AUDMONKEY_DLL_API WholeMenu {
   ...
   struct AUDMONKEY_DLL_API MenuItem final
   ...
   // in each call.
   class AUDMONKEY_DLL_API FinderScope : ValueRestorer< CommandHandlerFinder >
   {
   ...
   struct AUDMONKEY_DLL_API CommandItem final : SingleItem {
   ...
   struct AUDMONKEY_DLL_API CommandGroupItem final : SingleItem {
   ...
   {
      using Appender = std::function< void( AudMonkeyProject&, wxMenu& ) >;
   ...
   // stable across AudMonkey versions.
   template< typename... Args >
   ...
   // The name is untranslated.  Try to keep the name stable across AudMonkey
   // versions.
   ...
   struct AUDMONKEY_DLL_API AttachedItem final
   {
