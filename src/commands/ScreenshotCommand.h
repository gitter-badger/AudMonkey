...
private:
   ...
   void CaptureEffects(const CommandContext & Context, AudMonkeyProject * pProject, const wxString &fileName );
   void CaptureScriptables(const CommandContext & Context, AudMonkeyProject * pProject, const wxString &fileName );
   void CapturePreferences(const CommandContext & Context, AudMonkeyProject * pProject, const wxString &fileName );
   bool Capture(
   ...
   wxRect GetTrackRect( AudMonkeyProject * pProj, TrackPanel * panel,int n);
   wxString WindowFileName(AudMonkeyProject * proj, wxTopLevelWindow *w);

public:
   static ScreenshotCommand * mpShooter;
   static void (*mIdleHandler)(wxIdleEvent& event);
   static void SetIdleHandler( AudMonkeyProject &project );
   static bool MayCapture( wxDialog * pDlg );

   void CaptureWindowOnIdle( const CommandContext & context, wxWindow * pWin );
   wxTopLevelWindow *GetFrontWindow(AudMonkeyProject *project);
};
