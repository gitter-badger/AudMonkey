...
\class AudMonkeyLogger
\brief AudMonkeyLogger is a thread-safe logger class
...
//     AudMonkey may crash when terminating.  It's not fully understood why this occurs
...
// 2)  By providing an AudMonkey specific logging class, it can be made thread-safe and,
//     as such, can be used by the ever growing threading within AudMonkey.
//
enum
{
...
AudMonkeyLogger *AudMonkeyLogger::Get()
{
   ...
   std::call_once( flag, []{
      ...
      std::unique_ptr < wxLog > // DELETE any previous logger
         { wxLog::SetActiveTarget(safenew AudMonkeyLogger) };
   } );

   // Use dynamic_cast so that we get a NULL ptr in case our logger
   // is no longer the target.
   return dynamic_cast<AudMonkeyLogger *>(wxLog::GetActiveTarget());
}

AudMonkeyLogger::AudMonkeyLogger()
:  wxEvtHandler(),
   wxLog()
{
   mText = NULL;
...
AudMonkeyLogger::~AudMonkeyLogger()  = default;

void AudMonkeyLogger::Flush()
{
...
void AudMonkeyLogger::DoLogText(const wxString & str)
{
   if (!wxIsMainThread()) {
      wxMutexGuiEnter();
   }
...
bool AudMonkeyLogger::SaveLog(const wxString &fileName) const
{
...
void AudMonkeyLogger::Show(bool show)
{
   ...
   Destroy_ptr<wxFrame> frame
      { safenew wxFrame(NULL, wxID_ANY, _("AudMonkey Log")) };
   ...
   frame->Bind(wxEVT_CLOSE_WINDOW,
                  wxCloseEventHandler(AudMonkeyLogger::OnCloseWindow),
                  this);

   frame->Bind(   wxEVT_COMMAND_MENU_SELECTED,
                  &AudMonkeyLogger::OnSave,
                  this, LoggerID_Save);
   frame->Bind(   wxEVT_COMMAND_MENU_SELECTED,
                  &AudMonkeyLogger::OnClear,
                  this, LoggerID_Clear);
   frame->Bind(   wxEVT_COMMAND_MENU_SELECTED,
                  &AudMonkeyLogger::OnClose,
                  this, LoggerID_Close);
...
wxString AudMonkeyLogger::GetLog(int count)
{
   if (count == 0)
   {
      return mBuffer;
...
void AudMonkeyLogger::OnCloseWindow(wxCloseEvent & WXUNUSED(e))
{
...
void AudMonkeyLogger::OnClose(wxCommandEvent & WXUNUSED(e))
{
   wxCloseEvent dummy;
   OnCloseWindow(dummy);
}

void AudMonkeyLogger::OnClear(wxCommandEvent & WXUNUSED(e))
{
   ClearLog();
}

void AudMonkeyLogger::OnSave(wxCommandEvent & WXUNUSED(e))
{
...
void AudMonkeyLogger::UpdatePrefs()
{
   if (mFrame) {
