...
AudMonkeyFileConfig::AudMonkeyFileConfig(
...
AudMonkeyFileConfig::~AudMonkeyFileConfig() = default;

std::unique_ptr<AudMonkeyFileConfig> AudMonkeyFileConfig::Create(
   const wxString& appName,
   const wxString& vendorName,
   const wxString& localFilename,
   const wxString& globalFilename,
...
)
{
   ...
   auto result = std::unique_ptr<AudMonkeyFileConfig>{
      safenew AudMonkeyFileConfig{
         appName, vendorName, localFilename, globalFilename, style, conv } };
   result->Init();
   return result;
}

void AudMonkeyFileConfig::Warn()
{
   wxDialogWrapper dlg(nullptr, wxID_ANY, XO("AudMonkey Configuration Error"));
   ...
   S.StartVerticalLay(wxEXPAND, 1);
   {
      ...
      S.StartHorizontalLay(wxALIGN_RIGHT, 0);
      {
         S.AddFixedText(
            XO("The following configuration file could not be accessed:\n\n"
               ...
               "If you choose to \"Quit AudMonkey\", your project may be left in an unsaved "
      ...
      S.StartHorizontalLay(wxALIGN_RIGHT, 0);
      {
         ...
         b = S.Id(wxID_CANCEL).AddButton(XXO("&Quit AudMonkey"));
   ...
   switch (dlg.ShowModal())
   {
      case wxID_HELP:
         ...
         OpenInDefaultBrowser("https://" +
                              ...
                              "Error:_AudMonkey_settings_file_unwritable");
      break;
