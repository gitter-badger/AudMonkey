...
\class AudMonkeyApp
\brief AudMonkeyApp is the 'main' class for AudMonkey
...
void PopulatePreferences()
{
   ...
   if (fn.FileExists())   // it will exist if the (win) installer put it there
   {
      ...
      auto pIni =
         AudMonkeyFileConfig::Create({}, {}, fullPath, {},
   ...
   if (langCode.empty())
      langCode =
         Languages::GetSystemLanguageCode(FileNames::AudMonkeyPathList());
...
"Reset Preferences?\n\nThis is a one-time question, after an 'install' where you asked to have the Preferences reset.");
      int action = AudMonkeyMessageBox(
         prompt,
         XO("Reset AudMonkey Preferences"),
         wxYES_NO, NULL);
   ...
   // In AUdacity 2.1.0 support for the legacy 1.2.x preferences (depreciated since AudMonkey
   // 1.3.1) is dropped. As a result we can drop the import flag
   // first time this version of AudMonkey is run we try to migrate
   // old preferences.
   ...
   // (meaning the changes won't mess audmonkey up if the user goes back to an earlier version)
...
static void QuitAudMonkey(bool bForce)
{
...
class GnomeShutdown
{
 public:
   GnomeShutdown()
   {
      mArgv[0].reset(strdup("AudMonkey"));
...
IMPLEMENT_APP_NO_MAIN(AudMonkeyApp)
...
#else
IMPLEMENT_APP(AudMonkeyApp)
#endif

#ifdef __WXMAC__

// in response of an open-document apple event
void AudMonkeyApp::MacOpenFile(const wxString &fileName)
{
   ofqueue.push_back(fileName);
}

// in response of a print-document apple event
void AudMonkeyApp::MacPrintFile(const wxString &fileName)
{
   ofqueue.push_back(fileName);
}

// in response of a open-application apple event
void AudMonkeyApp::MacNewFile()
{
   if (!gInited)
      return;
...
#define kAudMonkeyAppTimerID 0

BEGIN_EVENT_TABLE(AudMonkeyApp, wxApp)
   EVT_QUERY_END_SESSION(AudMonkeyApp::OnQueryEndSession)
   EVT_END_SESSION(AudMonkeyApp::OnEndSession)

   EVT_TIMER(kAudMonkeyAppTimerID, AudMonkeyApp::OnTimer)
#ifdef __WXMAC__
   EVT_MENU(wxID_NEW, AudMonkeyApp::OnMenuNew)
   EVT_MENU(wxID_OPEN, AudMonkeyApp::OnMenuOpen)
   EVT_MENU(wxID_ABOUT, AudMonkeyApp::OnMenuAbout)
   EVT_MENU(wxID_PREFERENCES, AudMonkeyApp::OnMenuPreferences)
#endif
   ...
   // event
   EVT_MENU(wxID_EXIT, AudMonkeyApp::OnMenuExit)

#ifndef __WXMSW__
   EVT_SOCKET(ID_IPC_SERVER, AudMonkeyApp::OnServerEvent)
   EVT_SOCKET(ID_IPC_SOCKET, AudMonkeyApp::OnSocketEvent)
#endif

   // Recent file event handlers.
   EVT_MENU(FileHistory::ID_RECENT_CLEAR, AudMonkeyApp::OnMRUClear)
   EVT_MENU_RANGE(FileHistory::ID_RECENT_FIRST, FileHistory::ID_RECENT_LAST,
      AudMonkeyApp::OnMRUFile)

   // Handle AppCommandEvents (usually from a script)
   EVT_APP_COMMAND(wxID_ANY, AudMonkeyApp::OnReceiveCommand)

   // Global ESC key handling
   EVT_KEY_DOWN(AudMonkeyApp::OnKeyDown)
END_EVENT_TABLE()
...
bool AudMonkeyApp::MRUOpen(const FilePath &fullPathStr) {
   // Most of the checks below are copied from ProjectManager::OpenFiles.
   // - some rationalisation might be possible.

   AudMonkeyProject *proj = GetActiveProject();
   ...
   if (!fullPathStr.empty())
   {
      ...
      if (wxFile::Exists(fullPathStr))
      {
         ...
         // Test here even though AudMonkeyProject::OpenFile() also now checks, because
...
bool AudMonkeyApp::SafeMRUOpen(const wxString &fullPathStr)
{
   return GuardedCall< bool >( [&]{ return MRUOpen( fullPathStr ); } );
}

void AudMonkeyApp::OnMRUClear(wxCommandEvent& WXUNUSED(event))
{
...
void AudMonkeyApp::OnTimer(wxTimerEvent& WXUNUSED(event))
{
   // Filenames are queued when AudMonkey receives a few of the
      ...
      if (ofqueue.size()) {
         ...
         while (ofqueue.size()) {
            ...
            if (name.empty()) {
               ...
               AudMonkeyProject *project = GetActiveProject();
               if (project) {
...
void AudMonkeyApp::OnFatalException()
{
...
bool AudMonkeyApp::OnExceptionInMainLoop()
{
...
bool AudMonkeyApp::OnInit()
{
   ...
   if ( !ProjectFileIO::InitializeSQL() )
      this->CallAfter([]{
         ::AudMonkeyMessageBox(
            XO("SQLite library failed to initialize.  AudMonkey cannot continue.") );
         QuitAudMonkey( true );
      });


   // cause initialization of wxWidgets' global logger target
   (void) AudMonkeyLogger::Get();
   ...
   gtk_rc_parse_string("style \"audmonkey\" {\n"
...
// DA: App name
   ...
   wxString appName = wxT("AudMonkey");
#else
   wxString appName = wxT("DarkAudMonkey");
#endif
   ...
   if (!envTempDir.empty()) {
      ...
      TempDirectory::SetDefaultTempDir( wxString::Format(
         wxT("%s/audmonkey-%s"), envTempDir, wxGetUserId() ) );
   } else {
      /* On Unix systems, the default temp dir is in /var/tmp. */
      TempDirectory::SetDefaultTempDir( wxString::Format(
         wxT("/var/tmp/audmonkey-%s"), wxGetUserId() ) );
   }
...
// DA: Path env variable.
   ...
   FileNames::AddUniquePathToPathList(::wxGetCwd(), audmonkeyPathList);

   wxString progPath = wxPathOnly(argv[0]);
   FileNames::AddUniquePathToPathList(progPath, audmonkeyPathList);
   // Add the path to modules:
   FileNames::AddUniquePathToPathList(progPath + L"/lib/audmonkey", audmonkeyPathList);
   ...
   FileNames::AddUniquePathToPathList(FileNames::ModulesDir(),
      audmonkeyPathList);
   FileNames::AddUniquePathToPathList(wxString::Format(wxT("%s/share/audmonkey"),
      wxT(INSTALL_PREFIX)),
      audmonkeyPathList);
...
// JKC Bug 1220: Use path based on home directory on WXMAC
   ...
   // On Mac and Windows systems, use the directory which contains AudMonkey.
#ifdef __WXMSW__
   // On Windows, the path to the AudMonkey program is in argv[0]
   wxString progPath = wxPathOnly(argv[0]);
   FileNames::AddUniquePathToPathList(progPath, audmonkeyPathList);
   FileNames::AddUniquePathToPathList(progPath + wxT("\\Languages"), audmonkeyPathList);
...
#ifdef __WXMAC__
   // On Mac OS X, the path to the AudMonkey program is in argv[0]
   wxString progPath = wxPathOnly(argv[0]);

   FileNames::AddUniquePathToPathList(progPath, audmonkeyPathList);
   // If AudMonkey is a "bundle" package, then the root directory is
   // the great-great-grandparent of the directory containing the executable.
   //FileNames::AddUniquePathToPathList(progPath + wxT("/../../../"), audmonkeyPathList);

   // These allow for searching the "bundle"
   FileNames::AddUniquePathToPathList(
      progPath + wxT("/../"), audmonkeyPathList);
   FileNames::AddUniquePathToPathList(
      progPath + wxT("/../Resources"), audmonkeyPathList);
   ...
   TempDirectory::SetDefaultTempDir( wxString::Format(
      wxT("%s/Library/Application Support/audmonkey/SessionData"), tmpDirLoc) );
   ...
   FileNames::SetAudMonkeyPathList( std::move( audmonkeyPathList ) );
   ...
   {
      wxFileName configFileName(FileNames::DataDir(), wxT("audmonkey.cfg"));
      auto appName = wxTheApp->GetAppName();
      InitPreferences( AudMonkeyFileConfig::Create(
   ...
   // AudMonkey is running, we must return from OnInit() to wxWidgets before
...
bool AudMonkeyApp::InitPart2()
{
   ...
   wxImage logoimage((const char **)AudMonkeyLogoWithName_xpm);
   ...
   AudMonkeyProject *project;
   {
      ...
      // as where AudMonkey project will appear.
      wxRect wndRect;
      ...
      temporarywindow.SetTitle(_("AudMonkey is starting up..."));
      ...
      fileMenu->Append(wxID_ABOUT, _("&About AudMonkey..."));
   ...
   // Workaround Bug 1377 - Crash after AudMonkey starts and low disk space warning appears
   ...
   mTimer.SetOwner(this, kAudMonkeyAppTimerID);
   mTimer.Start(200);
...
#if defined(__WXMAC__)
   // The first time this version of AudMonkey is run or when the preferences
   // are reset, execute the "tccutil" command to reset the microphone permissions
   // currently assigned to AudMonkey.  The end result is that the user will be
   // prompted to approve/deny AudMonkey access (again).
   //
   // This should resolve confusion of why AudMonkey appears to record, but only
   // gets silence due to AudMonkey being denied microphone access previously.
   bool permsReset = false;
   gPrefs->Read(wxT("/MicrophonePermissionsReset"), &permsReset, false);
   if (!permsReset) {
      system("tccutil reset Microphone org.audmonkeyteam.audmonkey");
...
void AudMonkeyApp::InitCommandHandler()
{
...
void AudMonkeyApp::OnReceiveCommand(AppCommandEvent &event)
{
   wxASSERT(NULL != mCmdHandler);
   mCmdHandler->OnReceiveCommand(event);
}

void AudMonkeyApp::OnKeyDown(wxKeyEvent &event)
{
...
bool AudMonkeyApp::InitTempDir()
{
   ...
   if (temp.empty()) {
      // Failed
      if( !TempDirectory::IsTempDirectoryNameOK( tempFromPrefs ) ) {
         AudMonkeyMessageBox(XO(
"AudMonkey could not find a safe place to store temporary files.\nAudMonkey needs a place where automatic cleanup programs won't delete the temporary files.\nPlease enter an appropriate directory in the preferences dialog."));
      } else {
         AudMonkeyMessageBox(XO(
"AudMonkey could not find a place to store temporary files.\nPlease enter an appropriate directory in the preferences dialog."));
      }
      ...
      AudMonkeyMessageBox(XO(
"AudMonkey is now going to exit. Please launch AudMonkey again to use the new temporary directory."));
      return false;
   }
...
// Return true if there are no other instances of AudMonkey running,
// false otherwise.

bool AudMonkeyApp::CreateSingleInstanceChecker(const wxString &dir)
{
   wxString name = wxString::Format(wxT("audmonkey-lock-%s"), wxGetUserId());
   mChecker.reset();
   auto checker = std::make_unique<wxSingleInstanceChecker>();

   auto runningTwoCopiesStr = XO("Running two copies of AudMonkey simultaneously may cause\ndata loss or cause your system to crash.\n\n");
   ...
   if (!checker->Create(name, dir))
   {
      ...
      auto prompt = XO(
"AudMonkey was not able to lock the temporary files directory.\nThis folder may be in use by another copy of AudMonkey.\n")
         + runningTwoCopiesStr
         + XO("Do you still want to start AudMonkey?");
      int action = AudMonkeyMessageBox(
         prompt,
      ...
      // to an already active AudMonkey.  If successful, we send
      // the first command line argument (the audio file name)
      // to that AudMonkey for processing.
      wxClient client;
      ...
      for (int i = 0; i < 50; i++)
      {
         ...
         if (conn)
         {
            ...
            {
               // Send an empty string to force existing AudMonkey to front
      ...
      // There is another copy of AudMonkey running.  Force quit.

      auto prompt =  XO(
"The system has detected that another copy of AudMonkey is running.\n")
         + runningTwoCopiesStr
         + XO(
"Use the New or Open commands in the currently running AudMonkey\nprocess to open multiple projects simultaneously.\n");
      AudMonkeyMessageBox(
         prompt, XO("AudMonkey is already running"),
         wxOK | wxICON_ERROR);
...
bool AudMonkeyApp::CreateSingleInstanceChecker(const wxString &dir)
{
   ...
   // time AudMonkey has been run during this boot of the system. In this
   ...
   if (isServer)
   {
      ...
      {
         AudMonkeyMessageBox(
            XO("The AudMonkey IPC server failed to initialize.\n\n"
               "This is likely due to a resource shortage\n"
               "and a reboot may be required."),
            XO("AudMonkey Startup Failure"),
            wxOK | wxICON_ERROR);
   ...
   // If we get here, then AudMonkey is currently active. So, we connect
   ...
   // Attempt to connect to an active AudMonkey.
   ...
   // go to AudMonkeyApp::MacOpenFile. Forward the file names to the prior
...
void AudMonkeyApp::OnServerEvent(wxSocketEvent & /* evt */)
{
   wxSocketBase *sock;
...
void AudMonkeyApp::OnSocketEvent(wxSocketEvent & evt)
{
...
std::unique_ptr<wxCmdLineParser> AudMonkeyApp::ParseCommandLine()
{
   ...
   /*i18n-hint: This controls the number of bytes that AudMonkey will
   ...
   /*i18n-hint: This runs a set of automatic tests on AudMonkey itself */
   parser->AddSwitch(wxT("t"), wxT("test"), _("run self diagnostics"));

   /*i18n-hint: This displays the AudMonkey version */
   parser->AddSwitch(wxT("v"), wxT("version"), _("display AudMonkey version"));

   /*i18n-hint: This is a list of one or more files that AudMonkey
...
void AudMonkeyApp::OnQueryEndSession(wxCloseEvent & event)
{
   bool mustVeto = false;
...
void AudMonkeyApp::OnEndSession(wxCloseEvent & event)
{
...
void AudMonkeyApp::OnMenuAbout(wxCommandEvent & /*event*/)
{
...
void AudMonkeyApp::OnMenuNew(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
...
void AudMonkeyApp::OnMenuOpen(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
void AudMonkeyApp::OnMenuPreferences(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
void AudMonkeyApp::OnMenuExit(wxCommandEvent & event)
{
   // This function shadows a similar function
   // in Menus.cpp, but should only be used on the Mac platform
   // when no project windows are open. This check assures that
...
//BG: On Windows, associate the aup file type with AudMonkey
...
*/
#if defined(__WXMSW__) && !defined(__WXUNIVERSAL__) && !defined(__CYGWIN__)
void AudMonkeyApp::AssociateFileTypes()
{
   ...
   auto DefineType = [&](const wxString &type)
   {
      ...
      if (!root_key.empty())
      {
         associateFileTypes = wxT("AudMonkey.Project"); // Finally set value for the key
      }
   ...
   if (IsDefined(wxT(".aup3")) && IsDefined(wxT(".aup")) && IsDefined(wxT("AudMonkey.Project")))
   {
   ...
   int wantAssoc =
      AudMonkeyMessageBox(
         XO(
"AudMonkey project (.aup3) files are not currently \nassociated with AudMonkey. \n\nAssociate them, so they open on double-click?"),
         XO("AudMonkey Project Files"),
   ...
   else
   {
      ...
      associateFileTypes = wxT("AudMonkey.Project"); // Finally set value for .AUP key
      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
         associateFileTypes = wxT("AudMonkey Project File");
      }

      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project\\shell"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project\\shell\\open"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      }

      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project\\shell\\open\\command"));
      ...
      if (!associateFileTypes.Exists() ||
            (tmpRegAudPath.Find(wxT("audmonkey.exe")) >= 0))
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project\\shell\\open\\ddeexec"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project\\shell\\open\\ddeexec\\Application"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
      ...
      associateFileTypes.SetName(root_key + wxT("AudMonkey.Project\\shell\\open\\ddeexec\\Topic"));
      if (!associateFileTypes.Exists())
      {
         associateFileTypes.Create(true);
