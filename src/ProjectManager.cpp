...
#ifdef __WXGTK__
#include "../images/AudMonkeyLogoAlpha.xpm"
#endif

const int AudMonkeyProjectTimerID = 5200;

static AudMonkeyProject::AttachedObjects::RegisteredFactory sProjectManagerKey {
   []( AudMonkeyProject &project ) {
      return std::make_shared< ProjectManager >( project );
   }
};

ProjectManager &ProjectManager::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< ProjectManager >( sProjectManagerKey );
}

const ProjectManager &ProjectManager::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

ProjectManager::ProjectManager( AudMonkeyProject &project )
   : mProject{ project }
   , mTimer{ std::make_unique<wxTimer>(this, AudMonkeyProjectTimerID) }
{
...
// PRL:  This event type definition used to be in AudMonkeyApp.h, which created
...
BEGIN_EVENT_TABLE( ProjectManager, wxEvtHandler )
   ...
   EVT_TIMER(AudMonkeyProjectTimerID, ProjectManager::OnTimer)
END_EVENT_TABLE()
...
void ProjectManager::SaveWindowSize()
{
   ...
   else
   {
      if (foundIconizedProject) {
         ...
         // AudMonkey would have to be running with no projects open
...
{
public:
   DropTarget(AudMonkeyProject *proj)
   {
      mProject = proj;
...
private:
   ...
   extern void AddPages(   AudMonkeyProject * pProj, GuiFactory & Factory,  wxNotebook  * pNotebook );
#endif
...
void InitProjectWindow( ProjectWindow &window )
{
   ...
   // LLL: When AudMonkey starts or becomes active after returning from
...
#if !defined(__WXMAC__) && !defined(__WXX11__)
   {
      ...
      wxIcon ic{ wxICON(AudMonkeyLogo) };
#elif defined(__WXGTK__)
      wxIcon ic{wxICON(AudMonkeyLogoAlpha)};
#else
      wxIcon ic{};
      ic.CopyFromBitmap(theTheme.Bitmap(bmpAudMonkeyLogo48x48));
#endif
      window.SetIcon(ic);
   }
#endif
...
AudMonkeyProject *ProjectManager::New()
{
   wxRect wndRect;
   ...
   auto sp = std::make_shared< AudMonkeyProject >();
   AllProjects{}.Add( sp );
   auto p = sp.get();
   auto &project = *p;
   ...
   //   SetDropTarget(safenew AudMonkeyDropTarget(this));
   //   mTrackPanel->SetDropTarget(safenew AudMonkeyDropTarget(this));
...
void ProjectManager::OnCloseWindow(wxCloseEvent & event)
{
   ...
   // AudMonkeyProject::~AudMonkeyProject() and TrackPanel::OnTimer().
   ...
   if (!sbSkipPromptingForSave 
      ...
      if ( UndoManager::Get( project ).UnsavedChanges() ) {
         ...
         int result = AudMonkeyMessageBox(
            Message,
            Title,
   ...
   // before the TrackPanel is destroyed. This change was needed to stop AudMonkey
...
void ProjectManager::OpenFiles(AudMonkeyProject *proj)
{
   auto selectedFiles =
...
bool ProjectManager::SafeToOpenProjectInto(AudMonkeyProject &proj)
{
...
AudMonkeyProject *ProjectManager::OpenProject(
   AudMonkeyProject *pGivenProject, const FilePath &fileNameArg,
