...
bool AUPImportFileHandle::Open()
{
   ...
   if (ff.IsOpened())
   {
      ...
      if (!wxStrncmp(buf, wxT("AudMonkeyProject"), 15))
      {
         AudMonkeyMessageBox(
            XO("This project was saved by AudMonkey version 1.0 or earlier. The format has\n"
               "changed and this version of AudMonkey is unable to import the project.\n\n"
               "Use a version of AudMonkey prior to v3.0.0 to upgrade the project and then\n"
               "you may import it with this version of AudMonkey."),
            XO("Import Project"),
            wxOK | wxCENTRE,
            &GetProjectFrame(mProject));
...
bool AUPImportFileHandle::HandleXMLTag(const wxChar *tag, const wxChar **attrs)
{
   ...
   if (mCurrentTag.IsSameAs(wxT("project")) ||
       mCurrentTag.IsSameAs(wxT("audmonkeyproject")))
   {
...
#else
   AudMonkeyMessageBox(
      XO("MIDI tracks found in project file, but this build of AudMonkey does not include MIDI support, bypassing track."),
      XO("Project Import"),
...
bool AUPImportFileHandle::HandleWaveTrack(XMLTagHandler *&handler)
{
   ...
   // No active clip.  In early versions of AudMonkey, there was a single
...
bool AUPImportFileHandle::HandleEnvelope(XMLTagHandler *&handler)
{
   ...
   // Earlier versions of AudMonkey had a single implied waveclip, so for
