...
\brief An ImportFileHandle for LOF data
  ...
  a list of individual files to open in audmonkey in specific formats. Files may
...
public:
   ...
   std::unique_ptr<ImportFileHandle> Open(
      const FilePath &Filename, AudMonkeyProject *pProject) override;
};
...
{
public:
   LOFImportFileHandle( AudMonkeyProject *pProject,
...
LOFImportFileHandle::LOFImportFileHandle( AudMonkeyProject *pProject,
   const FilePath & name, std::unique_ptr<wxTextFile> &&file)
...
std::unique_ptr<ImportFileHandle> LOFImportPlugin::Open(
   const FilePath &filename, AudMonkeyProject *pProject)
{
   ...
   Tags * WXUNUSED(tags))
{
   ...
   // back to the caller.
   // Instead, it recursively calls AudMonkeyProject::Import for each file listed
   // in the .lof file.
   ...
   // The file may have "window" directives that cause NEW AudMonkeyProjects
...
static AudMonkeyProject *DoImportMIDIProject(
   AudMonkeyProject *pProject, const FilePath &fileName)
{
   AudMonkeyProject *pNewProject {};
   if ( !pProject )
