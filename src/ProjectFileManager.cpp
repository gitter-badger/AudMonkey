...
static const AudMonkeyProject::AttachedObjects::RegisteredFactory sFileManagerKey{
...
ProjectFileManager &ProjectFileManager::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< ProjectFileManager >( sFileManagerKey );
}

const ProjectFileManager &ProjectFileManager::Get( const AudMonkeyProject &project )
{
...
ProjectFileManager::ProjectFileManager( AudMonkeyProject &project )
: mProject{ project }
{
}
...
const char *const defaultHelpUrl =
   "FAQ:Errors_on_opening_or_recovering_an_AudMonkey_project";
  ...
  -> ReadProjectResults
{
   ...
   if (bParseSuccess)
   {
      if (discardAutosave)
         ...
         AudMonkeyMessageBox(
            resaved
               ? XO("This project was not saved properly the last time AudMonkey ran.\n\n"
...
'Save Project' is for an AudMonkey project, not an audio file.\n\
...
bool ProjectFileManager::SaveCopy(const FilePath &fileName /* = wxT("") */)
{
   ...
   do
   {
      ...
      {
         // Saving a copy of the project should never overwrite an existing project.
         AudMonkeyMessageDialog m(nullptr,
...
bool ProjectFileManager::OpenNewProject()
{
   ...
   if( !bOK )
   {
      ShowExceptionDialog(
         ...
         true, 
         audmonkey::ToWString(projectFileIO.GetLastLog()));
   }
   return bOK;
}
...
AudMonkeyProject *ProjectFileManager::OpenFile( const ProjectChooserFn &chooser,
   const FilePath &fileNameArg, bool addtohistory)
{
   ...
   // Vaughan, 2011-03-25: This was done previously in AudMonkeyProject::OpenFiles()
   //    and AudMonkeyApp::MRUOpen(), but if you open an aup file by double-clicking it
   ...
   //    This was reported in http://bugzilla.audmonkeyteam.org/show_bug.cgi?id=137#c17,
   ...
   // left over from an unsuccessful save or by previous versions of AudMonkey.
   // So we always refuse to open such files.
   if (fileName.Lower().EndsWith(wxT(".aup3.bak")))
   {
      AudMonkeyMessageBox(
         XO(
"You are trying to open an automatically created backup file.\nDoing this may result in severe data loss.\n\nPlease open the actual AudMonkey project file instead."),
...
AudMonkeyProject *ProjectFileManager::OpenProjectFile(
...
namespace {
bool ImportProject(AudMonkeyProject &dest, const FilePath &fileName)
{
