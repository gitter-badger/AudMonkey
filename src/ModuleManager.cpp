...
\brief Based on LoadLadspa, this code loads pluggable AudMonkey 
extension modules.  It also has the code to
invoke a function returning a replacement window,
i.e. an alternative to the usual interface, for AudMonkey.
...
void Module::ShowLoadFailureError(const wxString &Error)
{
   ...
   AudMonkeyMessageBox(XO("Unable to load the \"%s\" module.\n\nError: %s").Format(ShortName, Error),
...
bool Module::Load(wxString &deferredErrorMessage)
{
   ...
   if( moduleVersion != AUDACITY_VERSION_STRING) {
      AudMonkeyMessageBox(
         ...
         XO("Module Unsuitable"));
      wxLogMessage(wxT("The module \"%s\" is matched with AudMonkey version \"%s\". It will not be loaded."), mName, moduleVersion);
      mLib->Unload();
      return false;
   }
...
void ModuleManager::FindModules(FilePaths &files)
{
   ...
   for (const auto &path : audmonkeyPathList) {
   ...
   const FilePaths &files, FilePaths &decided, DelayedErrors &errors)
{
   ...
   for (const auto &file : files) {
      ...
      // and let the user decide for each one.
      {
         ...
         int action;
         action = ShowMultiDialog(msg, XO("AudMonkey Module Loader"),
            buttons,
            "",
