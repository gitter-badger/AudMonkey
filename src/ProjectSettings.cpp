...
namespace {
   void Notify( AudMonkeyProject &project, ProjectSettings::EventCode code )
   {
...
ProjectSettings &ProjectSettings::Get( AudMonkeyProject &project )
{
   return project.AttachedObjects::Get< ProjectSettings >(
      sProjectSettingsKey );
}

const ProjectSettings &ProjectSettings::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

ProjectSettings::ProjectSettings(AudMonkeyProject &project)
   : mProject{ project }
...
, mSnapTo( gPrefs->Read(wxT("/SnapTo"), SNAP_OFF) )
{
   ...
   if ( !wasDefined ) {
      ...
      // an entry for the default sample rate in audmonkey.cfg, AudMonkey can open
