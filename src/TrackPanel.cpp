...
\class TrackPanel::AudMonkeyTimer
...
TrackPanel &TrackPanel::Get( AudMonkeyProject &project )
{
   return project.AttachedWindows::Get< TrackPanel >( sKey );
}

const TrackPanel &TrackPanel::Get( const AudMonkeyProject &project )
{
   return Get( const_cast< AudMonkeyProject & >( project ) );
}

void TrackPanel::Destroy( AudMonkeyProject &project )
{
...
/// Gets the pointer to the AudMonkeyProject that
/// goes with this track panel.
AudMonkeyProject * TrackPanel::GetProject() const
{
...
void TrackPanel::OnTimer(wxTimerEvent& )
{
   ...
   AudMonkeyProject *const p = GetProject();
...
struct EmptyCell final : CommonTrackPanelCell {
   std::vector< UIHandlePtr > HitTest(
      const TrackPanelMouseState &, const AudMonkeyProject *) override
   { return {}; }
...
IsVisibleTrack::IsVisibleTrack(AudMonkeyProject *project)
   : mPanelRect {
