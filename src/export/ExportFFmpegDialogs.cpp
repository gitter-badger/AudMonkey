...
bool FFmpegPresets::SavePreset(ExportFFmpegOptions *parent, wxString &name)
{
...

   {
      ...
      {
         AudMonkeyMessageBox( XO("Please select format before saving a profile") );
         return false;
      }
      ...
      if (lb->GetSelection() < 0)
      {
         ...
         AudMonkeyMessageBox( XO("Please select codec before saving a profile") );
         return false;
      }
...
void FFmpegPresets::LoadPreset(ExportFFmpegOptions *parent, wxString &name)
{
   ...
   if (!preset)
   {
      AudMonkeyMessageBox( XO("Preset '%s' does not exist." ).Format(name));
      return;
   }
...
// may throw
{
   ...
   wxString dtdName = wxT("-//audmonkeyffmpegpreset-1.0.0//DTD//EN");
   wxString dtdURI =
      wxT("http://audmonkey.sourceforge.net/xml/audmonkeyffmpegpreset-1.0.0.dtd");

   xmlFile.Write(wxT("<!DOCTYPE "));
   xmlFile.Write(wxT("project "));
   xmlFile.Write(wxT("PUBLIC "));
   xmlFile.Write(wxT("\"-//audmonkeyffmpegpreset-1.0.0//DTD//EN\" "));
   xmlFile.Write(wxT("\"http://audmonkey.sourceforge.net/xml/audmonkeyffmpegpreset-1.0.0.dtd\" "));
   xmlFile.Write(wxT(">\n"));
}
...
void ExportFFmpegOptions::OnDeletePreset(wxCommandEvent& WXUNUSED(event))
{
   ...
   if (presetname.empty())
   {
      AudMonkeyMessageBox( XO("You can't delete a preset without name") );
      return;
   }
...
bool ExportFFmpegOptions::SavePreset(bool bCheckForOverwrite)
{
   ...
   if (name.empty())
   {
      AudMonkeyMessageBox( XO("You can't save a preset without a name") );
      return false;
   }
...
void ExportFFmpegOptions::OnExportPresets(wxCommandEvent& WXUNUSED(event))
{
   ...
   if( presets.Count() < 1)
   {
      AudMonkeyMessageBox( XO("No presets to export") );
      return;
   }
