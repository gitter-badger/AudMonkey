...
#ifndef __AUDMONKEY_EXPORT__
#define __AUDMONKEY_EXPORT__
...
class AUDMONKEY_DLL_API FormatInfo
{
   public:
      FormatInfo() {}
...
// ExportPlugin
//----------------------------------------------------------------------------
class AUDMONKEY_DLL_API ExportPlugin /* not final */
{
public:
   ...
   *  (working around a problem in wxWidgets for Mac; see bug 1600)
    ...
    */
   virtual ProgressResult Export(AudMonkeyProject *project,
...
wxDECLARE_EXPORTED_EVENT(AUDMONKEY_DLL_API,
   AUDMONKEY_FILE_SUFFIX_EVENT, wxCommandEvent);

class  AUDMONKEY_DLL_API Exporter final : public wxEvtHandler
{
public:
   ...
   struct AUDMONKEY_DLL_API RegisteredExportPlugin{
   ...
   static bool DoEditMetadata(AudMonkeyProject &project,
      const TranslatableString &title,
      const TranslatableString &shortUndoDescription, bool force);

   Exporter( AudMonkeyProject &project );
   virtual ~Exporter();
...
AUDMONKEY_DLL_API TranslatableString AudMonkeyExportCaptionStr();
AUDMONKEY_DLL_API TranslatableString AudMonkeyExportMessageStr();
...
AUDMONKEY_DLL_API void ShowExportErrorDialog(wxString ErrorCode,
   TranslatableString message = AudMonkeyExportMessageStr(),
   const TranslatableString& caption = AudMonkeyExportCaptionStr());
