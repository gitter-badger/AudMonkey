...
private:
   ...
   bool AddStrings(AudMonkeyProject *project, SNDFILE *sf, const Tags *tags, int sf_format);
   bool AddID3Chunk(
...
void ExportPCM::ReportTooBigError(wxWindow * pParent)
{
   ...
   auto message =
      ...
      "AudMonkey cannot do this, the Export was abandoned.");
...
// compute the size in advance.
   ...
   ShowErrorDialog(pParent, XO("Error Exporting"),
                  XO("Your exported WAV file has been truncated as AudMonkey cannot export WAV\n"
 ...
 */
ProgressResult ExportPCM::Export(AudMonkeyProject *project,
                                 std::unique_ptr<ProgressDialog> &pDialog,
                                 ...
                                 int subformat)
{
   ...
   auto updateResult = ProgressResult::Success;
   {
      ...
      {
         AudMonkeyMessageBox( XO("GSM 6.10 requires mono") );
      ...
      if (!sf_format_check(&info)) {
         AudMonkeyMessageBox( XO("Cannot export audio in this format.") );
         return ProgressResult::Cancelled;
      }
      ...
      if (!sf) {
         AudMonkeyMessageBox( XO("Cannot export audio to %s").Format( path ) );
         return ProgressResult::Cancelled;
      }
...
bool ExportPCM::AddStrings(AudMonkeyProject * WXUNUSED(project), SNDFILE *sf, const Tags *tags, int sf_format)
{
