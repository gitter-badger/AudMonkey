...
private:
   ...
   bool GetMetadata(AudMonkeyProject *project, const Tags *tags);
...
ProgressResult ExportFLAC::Export(AudMonkeyProject *project,
                        std::unique_ptr<ProgressDialog> &pDialog,
                        ...
                        int WXUNUSED(subformat))
{
   ...
   if (!f.Open(path, wxT("w+b"))) {
      AudMonkeyMessageBox( XO("FLAC export couldn't open %s").Format( path ) );
      return ProgressResult::Cancelled;
   }
...
bool ExportFLAC::GetMetadata(AudMonkeyProject *project, const Tags *tags)
{
