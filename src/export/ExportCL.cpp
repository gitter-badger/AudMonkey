/**********************************************************************
  ...
  This code allows AudMonkey to export data by piping it to an external
  program.
...
private:
   ...
   struct ExtendPath
   {
      ...
      ExtendPath()
      {
         ...
         wxString paths[] = {wxT("HKEY_LOCAL_MACHINE\\Software\\Lame for AudMonkey"),
                             wxT("HKEY_LOCAL_MACHINE\\Software\\FFmpeg for AudMonkey")};
         wxString npath;
         wxRegKey reg;
...
ProgressResult ExportCL::Export(AudMonkeyProject *project,
                                std::unique_ptr<ProgressDialog> &pDialog,
                                unsigned channels,
                                const wxFileNameWrapper &fName,
                                bool selectionOnly,
