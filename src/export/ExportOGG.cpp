...
private:
   ...
   bool FillComment(AudMonkeyProject *project, vorbis_comment *comment, const Tags *metadata);
};
...
ProgressResult ExportOGG::Export(AudMonkeyProject *project,
                       std::unique_ptr<ProgressDialog> &pDialog,
                       ...
                       int WXUNUSED(subformat))
{
   ...
   if (vorbis_encode_init_vbr(&info, numChannels, (int)(rate + 0.5), quality)) {
      ...
      AudMonkeyMessageBox( XO("Unable to export - rate or quality problem") );
      return ProgressResult::Cancelled;
   }
   ...
   if (!FillComment(project, &comment, metadata)) {
      AudMonkeyMessageBox( XO("Unable to export - problem with metadata") );
      return ProgressResult::Cancelled;
   }
   ...
   if (vorbis_analysis_init(&dsp, &info) ||
      ...
      AudMonkeyMessageBox( XO("Unable to export - problem initialising") );
      return ProgressResult::Cancelled;
   }
   ...
   if (ogg_stream_init(&stream, rand())) {
      AudMonkeyMessageBox( XO("Unable to export - problem creating stream") );
      return ProgressResult::Cancelled;
   }
   ...
   if(vorbis_analysis_headerout(&dsp, &comment, &bitstream_header, &comment_header,
      ...
      AudMonkeyMessageBox( XO("Unable to export - problem with packets") );
      return ProgressResult::Cancelled;
   }
   ...
   while (ogg_stream_flush(&stream, &page)) {
      if ( outFile.Write(page.header, page.header_len).GetLastError() ||
         ...
         AudMonkeyMessageBox( XO("Unable to export - problem with file") );
         return ProgressResult::Cancelled;
      }
   }
...
bool ExportOGG::FillComment(AudMonkeyProject *project, vorbis_comment *comment, const Tags *metadata)
{
