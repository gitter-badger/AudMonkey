...
ErrorReportDialog::ErrorReportDialog(
   ...
   const wxString& log, const bool modal)
    ...
    , mIsModal(modal)
{
   audmonkey::sentry::Exception ex = audmonkey::sentry::Exception::Create(
      audmonkey::ToUTF8(dlogTitle.Debug()),message.Debug());

   if (!log.empty())
      ex.AddData("log", log);

   mReport = std::make_unique<audmonkey::sentry::Report> (ex);
   ...
   S.StartHorizontalLay(wxEXPAND, 0);
   {
      ...
      S.StartVerticalLay(wxEXPAND, 0);
      {
         ...
         S.AddVariableText(XO(
               "Click \"Send\" to submit the report to AudMonkey. This information is collected anonymously."))
            ->SetFont(textFont);
...
void ErrorReportDialog::OnSend(wxCommandEvent& event)
{
   ...
   mReport->AddUserComment(audmonkey::ToUTF8(mCommentsControl->GetValue()));
