...
const wxString HelpSystem::HelpHostname = wxT("alphamanual.audmonkeyteam.org");
const wxString HelpSystem::HelpServerHomeDir = wxT("/man/");
const wxString HelpSystem::HelpServerManDir = wxT("/man/");
#else
const wxString HelpSystem::HelpHostname = wxT("manual.audmonkeyteam.org");
...
public:
   ...
   virtual ~HtmlTextHelpDialog()
   {
      ...
      MakeModal( false );
#endif
      // On Windows, for some odd reason, the AudMonkey window will be sent to
                          ...
                          bool bModal)
{
   ...
   // scripts/mw2html_audmonkey/mw2html.py
...
// DA: No bundled help, by default, and different quick-help URL.
      ...
      localHelpPage = wxFileName(FileNames::HtmlHelpDir(), releasePageName).GetFullPath();
      webHelpPath = L"http://www.darkaudmonkey.com/";
#else
