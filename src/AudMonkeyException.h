#ifndef __AUDMONKEY_EXCEPTION__
#define __AUDMONKEY_EXCEPTION__

/*!********************************************************************

 AudMonkey: A Digital Audio Editor

 @file AudMonkeyException.h
 @brief Declare abstract class AudMonkeyException, some often-used subclasses, and @ref GuardedCall
...
{
    Internal, //!< Indicates internal failure from AudMonkey.
 ...
 */
class AudMonkeyException /* not final */
{
public:
   AudMonkeyException() {}
   virtual ~AudMonkeyException() = 0;
...
protected:
   ...
   AudMonkeyException( const AudMonkeyException& ) = default;

   //! Don't allow moves of this class or subclasses
   // see https://bugzilla.audacityteam.org/show_bug.cgi?id=2442
   AudMonkeyException( AudMonkeyException&& ) = delete;

   //! Disallow assignment
   AudMonkeyException &operator = ( const AudMonkeyException & ) = delete;
};

//! Abstract AudMonkeyException subclass displays a message, specified by further subclass
...
class AUDMONKEY_DLL_API MessageBoxException /* not final */
   : public AudMonkeyException
{
   //! Privatize the inherited function
   using AudMonkeyException::DelayedHandlerAction;
...
class AUDMONKEY_DLL_API SimpleMessageBoxException /* not final */
...
{
   void operator () (AudMonkeyException *pException) const
   {
      if ( pException )
...
template <typename R> struct SimpleGuard
{
   explicit SimpleGuard(
      ...
      : m_value{ value } {}
   R operator () ( AudMonkeyException * ) const { return m_value; }
   const R m_value;
};
...
template<> struct SimpleGuard<bool>
{
   explicit SimpleGuard(
      ...
      : m_value{ value } {}
   bool operator () ( AudMonkeyException * ) const { return m_value; }
...
{
   SimpleGuard() {}
   void operator () ( AudMonkeyException * ) const {}
...
/*!
  ...
  If AudMonkeyException is handled, then it queues up a delayed handler action for execution later in
  the event loop at idle time, on the main thread; typically this informs the user of the error.

  The default delayed handler action is simply to invoke a method of the AudMonkeyException, but this
  ...
  takes pointer to AudMonkeyException, which is null when some other type of exception is caught;
  return value is converted to @b R
  @tparam F3 deduced type of delayed handler function, if a nondefault argument is given;
  takes pointer to AudMonkeyException, return value is unused
 */
template <
   typename R = void,
   ...
   typename F2 = SimpleGuard< R >, // function object
      // with signature R( AudMonkeyException * )

   typename F3 =
      DefaultDelayedHandlerAction // Any( AudMonkeyException * ), ignore return
>
//! Execute some code on any thread; catch any AudMonkeyException; enqueue error report on the main thread
R GuardedCall(
