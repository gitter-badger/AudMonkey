...
class AudMonkeyPrintout final : public wxPrintout
{
 public:
   AudMonkeyPrintout(wxString title,
...
bool AudMonkeyPrintout::OnPrintPage(int WXUNUSED(page))
{
   wxDC *dc = GetDC();
   if (!dc)
      return false;
...
bool AudMonkeyPrintout::HasPage(int page)
{
   return (page==1);
}

bool AudMonkeyPrintout::OnBeginDocument(int startPage, int endPage)
{
   return wxPrintout::OnBeginDocument(startPage, endPage);
}

void AudMonkeyPrintout::GetPageInfo(int *minPage, int *maxPage,
                                   int *selPageFrom, int *selPageTo)
{
   *minPage = 1;
   *maxPage = 1;
   ...
   TrackPanel &panel)
{
   ...
   AudMonkeyPrintout printout(name, tracks, panel);
