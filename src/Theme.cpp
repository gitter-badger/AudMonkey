...
\brief Based on ThemeBase, Theme manages image and icon resources.
   ...
   Theme adds the AudMonkey specific images to ThemeBase.
...
AUDMONKEY_DLL_API Theme theTheme;
...
// Must be wide enough for bmpAudMonkeyLogo. Use double width + 10.
...
//             .Format( FileName );
      ...
      {
         AudMonkeyMessageBox(
            XO("AudMonkey could not write file:\n  %s.")
               .Format( FileName ));
         return;
      }
      AudMonkeyMessageBox(
/* i18n-hint: A theme is a consistent visual style across an application's
 ...
 such as those on button controls.  AudMonkey can load and save alternative
 themes. */
         XO("Theme written to:\n  %s.")
            .Format( FileName ));
   }
   ...
   else
   {
      ...
      {
         AudMonkeyMessageBox(
            XO("AudMonkey could not open file:\n  %s\nfor writing.")
               .Format( FileName ));
         return;
      }
      ...
      {
         AudMonkeyMessageBox(
            XO("AudMonkey could not write images to file:\n  %s.")
               .Format( FileName ));
         return;
      }
      AudMonkeyMessageBox(
...
//   }
   ...
   if(  type == themeFromFile )
   {
      ...
      if( !wxFileExists( FileName ))
      {
         ...
         AudMonkeyMessageBox(
            XO("AudMonkey could not find file:\n  %s.\nTheme not loaded.")
               .Format( FileName ));
         return false;
      }
      if( !ImageCache.LoadFile( FileName, wxBITMAP_TYPE_PNG ))
      {
         AudMonkeyMessageBox(
            ...
            XO("AudMonkey could not load file:\n  %s.\nBad png format perhaps?")
               .Format( FileName ));
         return false;
      }
   }
   ...
   else
   {
      ...
      if( !ImageCache.LoadFile( InternalStream, wxBITMAP_TYPE_PNG ))
      {
         ...
         AudMonkeyMessageBox(
            XO(
"AudMonkey could not read its default theme.\nPlease report the problem."));
         return false;
      }
...
"Some required files in:\n  %s\nwere already present. Overwrite?")
   ...
   for(i = 0; i < (int)mImages.size(); i++)
   {
      if( (mBitmapFlags[i] & resFlagInternal)==0)
      {
         ...
         {
            AudMonkeyMessageBox(
               XO("AudMonkey could not save file:\n  %s")
                  .Format( FileName ));
            return;
         }
      }
   ...
   wxT("/GUI/Theme"),
   {
      ByColumns,
      {
         /* i18n-hint: describing the "classic" or traditional
            appearance of older versions of AudMonkey */
         XO("Classic")  ,
