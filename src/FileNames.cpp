...
used throughout AudMonkey into this one place.
...
const FileNames::FileType
     ...
     /* i18n-hint an AudMonkey project is the state of the program, stored as
     files that can be reopened to resume the session later */
   , FileNames::AudMonkeyProjects{ XO("AUP3 project files"), { wxT("aup3") }, true }
   ...
   const FilePath& file1, const FilePath& file2, bool overwrite)
{
   ...
   // http://bugzilla.audmonkeyteam.org/show_bug.cgi?id=1759
...
// The APP name has upercase first letter (so that Quit AudMonkey is correctly
// capitalised on Mac, but we want lower case APP name in paths.
// This function does that substitution, IF the last component of
// the path is 'AudMonkey'.
wxString FileNames::LowerCaseAppNameInPath( const wxString & dirIn){
   wxString dir = dirIn;
   // BUG 1577 Capitalisation of AudMonkey in path...
   if( dir.EndsWith( "AudMonkey" ) )
   {
      int nChars = dir.length() - wxString( "AudMonkey" ).length();
      dir = dir.Left( nChars ) + "audmonkey";
   }
   return dir;
}
...
FilePath FileNames::DataDir()
{
   ...
   //       creates a normal file as "$HOME/.audmonkey", while the former
   //       expects the ".audmonkey" portion to be a directory.
   if (gDataDir.empty())
   {
...
FilePath FileNames::HtmlHelpDir()
{
   ...
   //for mac this puts us within the .app: AudMonkey.app/Contents/SharedSupport/
   return wxFileName( exePath.GetPath()+wxT("/help/manual"), wxEmptyString ).GetFullPath();
#else
   //linux goes into /*prefix*/share/audmonkey/
...
wxFileNameWrapper FileNames::DefaultToDocumentsFolder(const wxString &preference)
{
   ...
   if ( bIsDefaultPath )
   {
      ...
      // The dialog that AudMonkey offers will allow the user to select a valid directory.
...
static FilePaths sAudMonkeyPathList;

const FilePaths &FileNames::AudMonkeyPathList()
{
   return sAudMonkeyPathList;
}

void FileNames::SetAudMonkeyPathList( FilePaths list )
{
   sAudMonkeyPathList = std::move( list );
}
