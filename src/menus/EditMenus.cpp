...
bool DoPasteText(AudMonkeyProject &project)
{
...
bool DoPasteNothingSelected(AudMonkeyProject &project)
{
   auto &tracks = TrackList::Get( project );
   auto &selectedRegion = ViewInfo::Get( project ).selectedRegion;
...
void OnUndo(const CommandContext &context)
{
   ...
   if (!ProjectHistory::Get( project ).UndoAvailable()) {
      AudMonkeyMessageBox( XO("Nothing to undo") );
      return;
   }
...
void OnRedo(const CommandContext &context)
{
   ...
   if (!ProjectHistory::Get( project ).RedoAvailable()) {
      AudMonkeyMessageBox( XO("Nothing to redo") );
      return;
   }
...
// TODO: The comment below suggests this workaround is obsolete.
      ...
      //
      // This workaround should be removed when AudMonkey updates to wxWidgets
      // 3.x which has a fix.
...
void OnPasteOver(const CommandContext &context)
{
   ...
   if((AudMonkeyProject::msClipT1 - AudMonkeyProject::msClipT0) > 0.0)
   {
      selectedRegion.setT1(
         selectedRegion.t0() +
         (AudMonkeyProject::msClipT1 - AudMonkeyProject::msClipT0));
      ...
      auto range = TrackList::Get( project ).Any<const LabelTrack>()
         + [&](const LabelTrack *pTrack){
            return LabelTrackView::Get( *pTrack ).IsTextSelected(
               ...
               const_cast<AudMonkeyProject&>(project)
            );
         };
      if ( !range.empty() )
         return true;
...
BaseItemSharedPtr EditMenu()
{
   ...
   Menu( wxT("Edit"), XXO("&Edit"),
      Section( "UndoRedo",
         ...
         [](AudMonkeyProject &project, wxMenu&) {
      ...
      // not appear in the Edit menu but instead under AudMonkey, consistent with
      // MacOS conventions.
      Section( "Preferences",
...
auto canSelectAll = [](const AudMonkeyProject &project){
   return MenuManager::Get( project ).mWhatIfNoSelection != 0; };
auto selectAll = []( AudMonkeyProject &project, CommandFlag flagsRqd ){
