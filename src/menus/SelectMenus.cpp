...
void DoNextPeakFrequency(AudMonkeyProject &project, bool up)
{
...
(AudMonkeyProject &project, double t0)
{
...
double GridMove
(AudMonkeyProject &project, double t, int minPix)
{
   const auto &settings = ProjectSettings::Get( project );
   auto rate = settings.GetRate();
   auto &viewInfo = ViewInfo::Get( project );
...
(AudMonkeyProject &project, double seekStep, TimeUnit timeUnit)
{
   auto &viewInfo = ViewInfo::Get( project );
...
(AudMonkeyProject &project, double direction, SelectionOperation operation,
 SeekInfo &info)
{
...
void DoCursorMove(
   AudMonkeyProject &project, double seekStep,
...
void DoBoundaryMove(AudMonkeyProject &project, int step, SeekInfo &info)
{
   auto &viewInfo = ViewInfo::Get( project );
   auto &tracks = TrackList::Get( project );
   auto &window = ProjectWindow::Get( project );
   ...
   Menu( wxT("Select"), XXO("&Select"),
      Section( "Basic",
         ...
         Menu( wxT("Region"), XXO("R&egion"),
            ...
            Section( "",
               // GA: AudMonkey had 'Store Re&gion' here previously. There is no
               ...
               // AudMonkey had 'Retrieve Regio&n' here previously.
