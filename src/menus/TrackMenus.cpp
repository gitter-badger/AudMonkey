...
void DoMixAndRender
(AudMonkeyProject &project, bool toNewTrack)
{
...
void DoPanTracks(AudMonkeyProject &project, float PanValue)
{
   auto &tracks = TrackList::Get( project );
   auto &window = ProjectWindow::Get( project );
...
void DoAlign
(AudMonkeyProject &project, int index, bool moveSel)
{
   auto &tracks = TrackList::Get( project );
...
//     audmonkey-src/win/Release/modules/
#define COLLECT_TIMING_DATA

// AudMonkey Score Align Progress class -- progress reports come here
...
enum{
   kAudMonkeySortByTime = (1 << 1),
   kAudMonkeySortByName = (1 << 2),
};

void DoSortTracks( AudMonkeyProject &project, int flags )
{
   ...
   for (auto iter = tracks.ListOfTracks::begin(),
      ...
      if ( !track->IsLeader() )
         ...
         for (ndx = 0; ndx < size;) {
            ...
            else if(flags & kAudMonkeySortByTime) {
...
void SetTrackGain(AudMonkeyProject &project, WaveTrack * wt, LWSlider * slider)
{
   wxASSERT(wt);
...
void SetTrackPan(AudMonkeyProject &project, WaveTrack * wt, LWSlider * slider)
{
   wxASSERT(wt);
   float newValue = slider->Get();
...
void OnNewTimeTrack(const CommandContext &context)
{
   ...
   if ( *tracks.Any<TimeTrack>().begin() ) {
      AudMonkeyMessageBox(
         XO(
"This version of AudMonkey only allows one time track for each project window.") );
      return;
   }
...
"Alignment error: input too short: MIDI from %.2f to %.2f secs, Audio from %.2f to %.2f secs.")
      ...
      //project.OnUndo(); // recover any changes to note track
      AudMonkeyMessageBox( XO("Internal error reported by alignment process.") );
   }
}
#endif /* EXPERIMENTAL_SCOREALIGN */
...
{
   auto &project = context.project;
   DoSortTracks(project, kAudMonkeySortByTime);
...
{
   auto &project = context.project;
   DoSortTracks(project, kAudMonkeySortByName);
