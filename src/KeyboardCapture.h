...
#ifndef __AUDMONKEY_KEYBOARD_CAPTURE__
#define __AUDMONKEY_KEYBOARD_CAPTURE__
...
/// don't actually come from wxWidgets, but are simulated by AudMonkey, as
/// translations of the EVT_CHAR_HOOK event); or, wxEVT_KEY_UP (really from
/// wxWidgets).
DECLARE_EXPORTED_EVENT_TYPE(AUDMONKEY_DLL_API, EVT_CAPTURE_KEY, -1);

namespace KeyboardCapture
{
   AUDMONKEY_DLL_API bool IsHandler(const wxWindow *handler);
   AUDMONKEY_DLL_API wxWindow *GetHandler();
   AUDMONKEY_DLL_API void Capture(wxWindow *handler);
   AUDMONKEY_DLL_API void Release(wxWindow *handler);
