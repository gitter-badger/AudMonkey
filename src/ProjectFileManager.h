...
{
public:
   static ProjectFileManager &Get( AudMonkeyProject &project );
   static const ProjectFileManager &Get( const AudMonkeyProject &project );

   // Open and close a file, invisibly, removing its Autosave blob
   static void DiscardAutosave(const FilePath &filename);

   explicit ProjectFileManager( AudMonkeyProject &project );
    ...
    *   are built into this build of AudMonkey, each with the relevant file
   ...
   using ProjectChooserFn = std::function<AudMonkeyProject&(bool)>;
    ...
    */
   static AudMonkeyProject *OpenFile( const ProjectChooserFn &chooser,
...
private:
   /*!
    ...
    @return if something was successfully opened, the project containing it; else null
    */
   AudMonkeyProject *OpenProjectFile(
