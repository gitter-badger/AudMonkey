...
#ifdef __WXMSW__
   #include "../images/AudMonkeyLogo.xpm"
#else
   #include "../images/AudMonkeyLogo48x48.xpm"
#endif

#include "commands/CommandManager.h"

#define AudMonkeyMixerBoardTitle XO("AudMonkey Mixer Board%s")
...
MixerTrackCluster::MixerTrackCluster(wxWindow* parent,
                                       MixerBoard* grandParent, AudMonkeyProject* project,
...
, mTrack{ pTrack }
{
   ...
   // (Still available in AudMonkey_UmixIt branch off 1.2.6.)
   ...
   if (GetWave()) {
      mMeter =
         safenew MeterPanel(mProject, // AudMonkeyProject* project,
...
void MixerTrackCluster::UpdateMeter(const double t0, const double t1)
{
      ...
      )
   {
      ...
      // Vaughan, 2011-02-04: Now that we're updating all meters from audmonkeyAudioCallback,
...
MixerBoardScrolledWindow::MixerBoardScrolledWindow(AudMonkeyProject* project,
...
MixerBoard::MixerBoard(AudMonkeyProject* pProject,
                        wxFrame* parent,
                        const wxPoint& pos /*= wxDefaultPosition*/,
                        const wxSize& size /*= wxDefaultSize*/)
: wxWindow(parent, -1, pos, size)
{
   ...
   mScrolledWindow =
      safenew MixerBoardScrolledWindow(
         pProject, // AudMonkeyProject* project,
...
void MixerBoard::OnTimer(wxCommandEvent &event)
{
   ...
   //v Vaughan, 2011-02-25: Moved this update back here from audmonkeyAudioCallback.
   //    See note there.
   // Vaughan, 2010-01-30:
   //    Since all we're doing here is updating the meters, I moved it to
   //    audmonkeyAudioCallback where it calls gAudioIO->mOutputMeter->UpdateDisplay().
...
MixerBoardFrame::MixerBoardFrame(AudMonkeyProject* parent)
...
#else
      wxIcon ic{wxICON(AudMonkeyLogo48x48)};
#endif
      SetIcon(ic);
   }
#endif
...
{
   AudMonkeyProject *project = mMixerBoard->mProject;
   auto &commandManager = CommandManager::Get( *project );
   commandManager.FilterKeyEvent(project, event, true);
}

void MixerBoardFrame::Recreate( AudMonkeyProject *pProject )
{
...
void MixerBoardFrame::SetWindowTitle()
{
   ...
   SetTitle(AudMonkeyMixerBoardTitle.Format(name).Translation());
}
...
AudMonkeyProject::AttachedWindows::RegisteredFactory sMixerBoardKey{
...
CommandHandlerObject &findCommandHandler(AudMonkeyProject &) {
