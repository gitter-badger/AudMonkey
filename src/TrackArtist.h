...
#ifndef __AUDMONKEY_TRACKARTIST__
#define __AUDMONKEY_TRACKARTIST__
...
class AUDMONKEY_DLL_API TrackArtist final : private PrefsListener {
...
extern AUDMONKEY_DLL_API int GetWaveYPos(float value, float min, float max,
                       int height, bool dB, bool outer, float dBr,
                       bool clip);
extern float FromDB(float value, double dBRange);
extern AUDMONKEY_DLL_API float ValueOfPixel(int yy, int height, bool offset,
                          bool dB, double dBRange, float zoomMin, float zoomMax);

#endif                          // define __AUDMONKEY_TRACKARTIST__
