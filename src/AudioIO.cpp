...
\brief AudioIO uses the PortAudio library to play and record sound.
  ...
  latency to the output device allows AudMonkey to stop audio output
  ...
  in the Audio callback (audmonkeyAudioCallback) for use by a time
  ...
  audio buffer is output by the audio callback. However, AudMonkey has
...
@endverbatim
  ...
  \li \c MidiTime() is an estimate in milliseconds of the current audio output time + 1s. In other words, what audmonkey track time corresponds to the audio (plus pause insertions) at the DAC output?
  ...
  *current* system time, but we really want the MidiTime(x) for some
  ...
  be to go to fixed size double buffer, but AudMonkey seems to work
...
int audmonkeyAudioCallback(const void *inputBuffer, void *outputBuffer,
...
AudioIO::AudioIO()
{
   ...
   if (err != paNoError) {
      ...
      // XXX: we are in libaudmonkey, popping up dialogs not allowed!  A
...
static PaSampleFormat AudMonkeyToPortAudioSampleFormat(sampleFormat format)
{
   switch(format) {
   case int16Sample:
      return paInt16;
                                   ...
                                   sampleFormat captureFormat)
{
   ...
   // JKC: YES that's right.  Internally AudMonkey uses float, and float has space for
   ...
   if( numCaptureChannels > 0)
   {
      ...
      captureParameters.sampleFormat =
         AudMonkeyToPortAudioSampleFormat(mCaptureFormat);
   ...
   for (unsigned int tries = 0; tries < maxTries; tries++) {
      mLastPaError = Pa_OpenStream( &mPortStreamV19,
                                    ...
                                    paNoFlag,
                                    audmonkeyAudioCallback, lpUserData );
   ...
   if (mPortStreamV19 != NULL && mLastPaError == paNoError) {
      wxPowerResource::Acquire(wxPOWER_RESOURCE_SCREEN, _("AudMonkey Audio"));
   }
#endif
                         ...
                         const AudioIOStartStreamOptions &options)
{
   ...
   if (mNumPlaybackChannels > 0)
   {
      ...
      // group determination should mimic what is done in audmonkeyAudioCallback()
   ...
   if(mNumPlaybackChannels > 0 || mNumCaptureChannels > 0) {
      ...
      // between the call of audmonkeyAudioCallback and the actual output of
   ...
   do
   {
      ...
      try
      {
         ...
         if( mNumCaptureChannels > 0 )
         {
            ...
            {
               AudMonkeyMessageBox( XO("Out of memory!") );
               return false;
            }
...
bool AudioIO::IsAvailable(AudMonkeyProject *project) const
{
...
void AudioIO::StopStream()
{
   ...
   // AudMonkey can deadlock if it tries to update meters while
   ...
   if (mStreamToken > 0) {
      ...
      if (mCaptureTracks.size() > 0)
      {
         ...
         for (unsigned int i = 0; i < mCaptureTracks.size(); i++) {
            ...
            // many parts of AudMonkey that are not effects or editing
...
void AudioIO::FillBuffers()
{
   ...
   auto delayedHandler = [this] ( AudMonkeyException * pException ) {
      ...
      // AudMonkeyException::DelayedHandlerAction prevents redundant message
      // boxes.
      StopStream();
   ...
   if (!mRecordingException &&
      ...
      // handler
      [this] ( AudMonkeyException *pException ) {
         if ( pException ) {
...
// output (DAC) time + 1s. In other words, what audmonkey track time
...
void AudioIO::AILAProcess(double maxPeak) {
   AudMonkeyProject *const proj = mOwningProject;
   if (proj && mAILAActive) {
...
)
{
   ...
   for(unsigned t = 0; t < numCaptureChannels; t++) {
      ...
      switch(mCaptureFormat) {
         ...
         case int24Sample:
            // We should never get here. AudMonkey's int24Sample format
