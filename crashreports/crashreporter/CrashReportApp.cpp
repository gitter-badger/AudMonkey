...
namespace
{
    ...
    void DoShowCrashReportFrame(const wxString& header, const wxString& dump, const std::function<bool(const wxString& comment)>& onSend)
    {
        ...
        auto frame = new wxFrame(
            nullptr, 
            wxID_ANY, 
            _("Problem Report for AudMonkey"),
            wxDefaultPosition, 
            wxDefaultSize, 
        ...
        if (onSend != nullptr)
        {
            ...
            mainLayout->Add(new wxStaticText(frame, wxID_ANY, _("Click \"Send\" to submit the report to AudMonkey. This information is collected anonymously.")), wxSizerFlags().Border(wxALL));
        }
