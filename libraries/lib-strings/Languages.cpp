...
{
   FilePaths results;
   FindFilesInPathList(code + L"/audmonkey.mo", pathList, results);
#if defined(__WXMAC__)
   FindFilesInPathList(code + L".lproj/audmonkey.mo", pathList, results);
#endif
   FindFilesInPathList(code + L"/LC_MESSAGES/audmonkey.mo", pathList, results);
...
/*         wxLogDebug(wxT("code=%s name=%s fullCode=%s name=%s -> %s"),
   ...
   // JKC: Adding language for simplified audmonkey.
   {
      wxString code;
      code = wxT("en-simple");
...
wxString SetLang( const FilePaths &pathList, const wxString & lang )
{
   ...
   using future1 = decltype(
      ...
      // xgettext when compiling the catalog template audmonkey.pot.
