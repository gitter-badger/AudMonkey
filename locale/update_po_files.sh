...
for i in *.po; do
    ...
    "Project-Id-Version: audmonkey 3.0.3\\n"' $i > TEMP; mv TEMP $i
done
echo ";; Updating the .po files"
sed 's/.*/echo "msgmerge --lang=& &.po audmonkey.pot -o &.po";\
msgmerge --no-wrap --lang=& &.po audmonkey.pot -o &.po;/g' LINGUAS | bash
...
echo ""
head -n 11 audmonkey.pot | tail -n 3
wc -l audmonkey.pot
