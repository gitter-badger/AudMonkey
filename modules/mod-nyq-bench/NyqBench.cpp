...
namespace {
CommandHandlerObject &findme(AudMonkeyProject&)
{
...
extern "C"
{
   ...
   // ModuleDispatch
   // is called by AudMonkey to initialize/terminate the module
...
void NyqBench::OnSave(wxCommandEvent & e)
{
   ...
   {
      AudMonkeyMessageBox(XO("Script was not saved."),
                   XO("Warning"),
                   wxICON_EXCLAMATION,
                   this);
      return;
...
void NyqBench::OnGo(wxCommandEvent & e)
{
   ...
   AudMonkeyProject *p = GetActiveProject();
   wxASSERT(p != NULL);
...
void NyqBench::OnAbout(wxCommandEvent & e)
{
   ...
   i.SetDescription(_("External AudMonkey module which provides a simple IDE for writing effects."));
...
void NyqBench::OnFindDialog(wxFindDialogEvent & e)
{
   ...
   if (pos == wxString::npos) {
      AudMonkeyMessageBox(XO("No matches found"),
...
bool NyqBench::Validate()
{
   if (mScript->GetLastPosition() > 0 && mScript->IsModified()) {
      int ans;
      ans = AudMonkeyMessageBox(XO("Code has been modified. Are you sure?"),
                         XO("Warning"),
...
//----------------------------------------------------------------------------
// Connects AudMonkey menu item to an action in this dll.
